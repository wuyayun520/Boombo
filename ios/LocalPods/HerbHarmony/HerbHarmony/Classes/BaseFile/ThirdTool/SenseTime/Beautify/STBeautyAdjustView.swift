...
class STBeautyAdjustView: UIView {
    ...
    private lazy var topItemView: STBeautyTopItemView = {
        ...
        view.frame = CGRect.init(x: 0, y: 50, width: kLet_senseContent, height: 50)
        view.delegate = self
        return view
    }()
    ...
    private lazy var itemCollectionView: STBeautyCollectionView = {
        let view = STBeautyCollectionView.init(frame: CGRect.init(x: 0, y: 50+18, width: kLet_senseContent, height: beautyContainerView.height-50-18-kLet_appMarginText), collectionViewLayout: UICollectionViewLayout())
        return view
    }()
    
    ///  滤镜列表
    private lazy var filterListView: STFilterListView = {
        let view = STFilterListView.init(frame: CGRect.init(x: kLet_senseContent, y: 50+18, width: kLet_senseContent, height: beautyContainerView.height-50-18-kLet_appMarginText))
        return view
    }()
    
    
    private lazy var beautySlider: STBeautySlider = {
        let view = STBeautySlider.init(frame: CGRect.init(x: 22, y: 0, width: kLet_senseContent-44, height: 40))
        view.minimumTrackTintColor = UIColor.alright()
        ...
        view.setThumbImage(UIImage.nearCounterval(name: "st_slider_thumb"), for: .normal)
...
extension STBeautyAdjustView: STBeautyTopItemViewDelegate {
    ...
    func show() {
        ...
        popView = TalkingRecognizerDelegate.init(frame: UIScreen.main.bounds)
    ...
    func changeTopItem(senderTag: Int) {
        ...
        if itemType == .Filter {
            ...
            self.beautyContainerView.setContentOffset(CGPoint.init(x: kLet_senseContent, y: 0), animated: true)
...
extension STBeautyAdjustView {
    ...
    func setupSubviews() {
        ...
        beautyContainerView.frame = CGRect.init(x: 0, y: 50, width: kLet_senseContent, height: self.height - 50)
