...
@objcMembers public class STLicHelper: NSObject {
    ...
    func checkLicenseFromLocal() -> Bool {
        if let path = ViewPodspecBundle.bundle.path(forResource: "SENSEME", ofType: "lic") {
...
extension STLicHelper {
    ...
    public func checkRemoteLicInfoWith(completion: @escaping Completionblock) {
        let reqModel = NumberryMReactiveCompatible.init()
    ...
    func checkLicFileMd5With(md5: NSString) -> Bool {
        ...
        let fileMd5: NSString = DataThen.shared.trademarkThroughPath(filePath: filePath as String) as NSString
