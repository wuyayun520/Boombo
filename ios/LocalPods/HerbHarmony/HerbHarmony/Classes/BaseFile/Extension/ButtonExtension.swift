...
/*: "recharge" :*/
fileprivate let str_rowMyForValue:String = "equalech"

/*: "recharge_ :*/
fileprivate let str_activeValue:String = "rawecharaw"
fileprivate let str_backgroundValue:[Character] = ["g","e","_"]

/*: ".png" :*/
fileprivate let str_contentCreateData:String = "add input choice tap stack.png"

/*: "_ :*/
fileprivate let str_cellValue:String = "_"

/*: .png" :*/
fileprivate let str_centerValue:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let str_modelValue:String = ".jpgview to let push picture"

/*: .jpg" :*/
fileprivate let str_moreViewData:[Character] = [".","j","p","g"]
...
enum LeadingModelImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class RoomThen: UIButton {
    ...
    var imageAlignment: LeadingModelImageAlignment = .left
...
extension UIButton {
    ...
    func contentBottomText(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        tantamount(interval: interval, topOffset: 0)
    }
    ...
    func tantamount(interval: CGFloat, topOffset: CGFloat) {
    ...
    func warningOf(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func backgroundBlock(color: UIColor, forState: UIControl.State) {
    ...
    func howeverFinish(urlStr: String,
                       //: placeImg: UIImage? = UIImage.placeImgSquare(),
                       placeImg: UIImage? = UIImage.currentPop(),
                       ...
                       loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_clickValue)
    ...
    func torso(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((str_rowMyForValue.replacingOccurrences(of: "equal", with: "r") + "arge")) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (str_rowMyForValue.replacingOccurrences(of: "equal", with: "r") + "arge"), with: (str_activeValue.replacingOccurrences(of: "raw", with: "r") + String(str_backgroundValue)) + "\(kLet_nameTopText)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.howeverFinish(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(str_contentCreateData.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(str_contentCreateData.suffix(4))), with: "_\(kLet_nameTopText)" + (String(str_centerValue)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(str_modelValue.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(str_modelValue.prefix(4))), with: "_\(kLet_nameTopText)" + (String(str_moreViewData)))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.howeverFinish(urlStr: newUrl, placeImg: nil)
    }
