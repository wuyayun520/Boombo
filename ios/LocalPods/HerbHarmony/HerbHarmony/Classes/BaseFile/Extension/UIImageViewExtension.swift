...
/*: "add_header_cookies" :*/
fileprivate let str_sectionValue:[Character] = ["a","d","d","_","h","e","a","d","e","r","_","c","o","o","k","i","e","s"]
...
public extension UIImageView {
    ...
    @objc func nearSquare(urlStr: String,
    ...
    internal func commentGesture(urlStr: String,
                                 //: placeImg: UIImage = UIImage.placeImgSquare(),
                                 placeImg: UIImage = UIImage.currentPop(),
    ...
    @objc func setStr(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_clickValue)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.showNumbereraction())
    }
    ...
    @objc func oreoCookieWhiteCompletion(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: DoingAddrTool.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(str_sectionValue)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.currentPop(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
