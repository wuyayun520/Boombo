...
- (NSData *)lengthPicAccountingData {
    ...
    UIImage *image = self.textMessage;
...
- (UIImage *)produce {
    ...
    return [UIImage imageWithData:[self lengthPicAccountingData]];
}
...
- (UIImage *)length:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self originQuote:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self isochronal:size];
...
- (CGSize)originQuote:(Boolean)isSession {
...
+ (NSData *)location:(UIImage *)image custom:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG key:scalesize system:NO];
...
- (UIImage*)custom:(CGSize)targetSize{
...
- (UIImage *)key:(CGSize)targetSize system:(BOOL)isScale{
...
- (UIImage *)isochronal:(CGSize)newSize {
