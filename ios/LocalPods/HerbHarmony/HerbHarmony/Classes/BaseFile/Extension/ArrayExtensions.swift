...
public class BindSafeArray<Element> {
...
public extension BindSafeArray {
    //: var first: Element? {
    var first: Element? {
        //: var result: Element?
        var result: Element?
...
public extension BindSafeArray {
    ...
    func filter(isIncluded: @escaping (Element)->Bool) ->BindSafeArray<Element> {
        //: pthread_rwlock_rdlock(&lock)
        pthread_rwlock_rdlock(&lock)
        ...
        return BindSafeArray(array: result)
    }
    ...
    func sorted(by areInIncreasingOrder: (Element, Element)->Bool)->BindSafeArray<Element> {
        //: pthread_rwlock_rdlock(&lock)
        pthread_rwlock_rdlock(&lock)
        //: let result = array.sorted(by: areInIncreasingOrder)
        let result = array.sorted(by: areInIncreasingOrder)
        //: let newArray = ThreadSafeArray<Element>(array: result)
        let newArray = BindSafeArray<Element>(array: result)
        //: pthread_rwlock_unlock(&lock)
        pthread_rwlock_unlock(&lock)
        //: return newArray
        return newArray
...
public extension BindSafeArray where Element: Equatable {
...
public extension BindSafeArray {
    ...
    static func +=(left: inout BindSafeArray, right: Element) {
        //: left.append(right)
        left.append(right)
    }
