...
/*: "http://stat. :*/
fileprivate let str_beginAppData:String = "hengineenginep"

/*: .com" :*/
fileprivate let str_shareTitle:String = ".comdate add white break"

/*: "http://" :*/
fileprivate let str_roomData:[Character] = ["h","t"]
fileprivate let str_mediumDataText:String = "if current viewtp://"

/*: "https://" :*/
fileprivate let str_arrayContent:[Character] = ["h","t","t","p","s",":","/","/"]

/*: "http://m. :*/
fileprivate let str_managerFillViewTitle:[Character] = ["h","t","t","p",":","/","/","m","."]

/*: "http:// :*/
fileprivate let str_screenName:String = "in data color let imagehttp://"

/*: "-1" :*/
fileprivate let str_clickData:[Character] = ["-","1"]

/*: "3,90" :*/
fileprivate let str_banValue:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let str_resultData:String = "1990-"
fileprivate let str_enterTitle:String = "0on-0on"

/*: "MM-dd-yyyy" :*/
fileprivate let str_successData:[Character] = ["M","M","-","d","d","-","y","y"]
fileprivate let str_sizeData:[Character] = ["y","y"]

/*: "id" :*/
fileprivate let str_makeValue:String = "ID"
...
class NumberryMReactiveCompatible: NSObject {
    ...
    var requestType: AftereffectRequestType?
    ...
    override init() {
        ...
        self.requestServer = DoingAddrTool.share.serverUrlStr
...
struct PopBaseResponse: HandyJSON {
    //: var errno: Int!
...
public struct MediaTalkingErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct FromMeasurable: HandyJSON {
    ...
    var CSConfig = SendMeasurable() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (str_beginAppData.replacingOccurrences(of: "engine", with: "t") + "://stat.") + "\(kLet_clickValue)" + (String(str_shareTitle.prefix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(str_roomData) + String(str_mediumDataText.suffix(5)))) && !statDomain.hasPrefix((String(str_arrayContent))) {
                //: return "http://".appending(statDomain)
                return (String(str_roomData) + String(str_mediumDataText.suffix(5))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(str_managerFillViewTitle)) + "\(kLet_clickValue)" + (String(str_shareTitle.prefix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(str_screenName.suffix(7))) + "\(urlDomain)"
        }
    }
...
public class ModelTransformable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [ChatViewMeasurable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [ChatViewMeasurable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [ChatViewMeasurable] = [] // 语音收费设置
    ...
    var vipSetting = [EffectMeasurable]() // vip皮肤
    ...
    var tabPageBanner = [CoverModelType]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = UserModelType() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class DeceasedPersonTransformable: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [StartTransformable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [StartTransformable]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var callPopupSetting = (str_banValue.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    var residualInfo = [ManagerHandyJSON]() // 套餐数量

    //: func updateResidualNum(type: Int, num: Int) {
    func nearDay(type: Int, num: Int) {
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.cellFormat(self.birthday ?? (str_resultData.capitalized + str_enterTitle.replacingOccurrences(of: "on", with: "1")), dateFormat: (String(str_successData) + String(str_sizeData)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.win().year else { return "0" }
...
struct ManagerHandyJSON: HandyJSON {
    //: var type = 0
...
struct BindModelType: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct StartTransformable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func compareEnable(compareTo: StartTransformable) -> Bool {
        //: return
        return
...
struct BridgeMeasurable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [RepairTransformable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [RepairTransformable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct RepairTransformable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [StartTransformable]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct SendMeasurable: HandyJSON {
...
struct EffectMeasurable: HandyJSON {
