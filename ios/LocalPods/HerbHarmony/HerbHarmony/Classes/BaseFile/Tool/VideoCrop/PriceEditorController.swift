...
fileprivate let str_availableTitle:[UInt8] = [0x2a,0x2d,0x2a,0x37,0x6b,0x20,0x2c,0x27,0x26,0x31,0x79,0x6a,0x63,0x2b,0x22,0x30,0x63,0x2d,0x2c,0x37,0x63,0x21,0x26,0x26,0x2d,0x63,0x2a,0x2e,0x33,0x2f,0x26,0x2e,0x26,0x2d,0x37,0x26,0x27]

private func modelButton(container num: UInt8) -> UInt8 {
    return num ^ 67
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let str_liveValue:String = "btnJQbEAlet location"
fileprivate let str_rowData:String = "B_etview let"
fileprivate let str_mText:[Character] = ["_","s","h","a","d","o","w"]

/*: "Edit" :*/
fileprivate let str_requestText:String = "Editbottom let model status"

/*: "Finish" :*/
fileprivate let str_labValue:String = "Finishindicator image self let gift"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let str_shareValue:String = "after visiblebtnY"
fileprivate let str_displayTitle:String = "_oedivtype table label make equal"

/*: "Cancel" :*/
fileprivate let str_makeName:String = "bubble sizeCancel"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let str_imageBottomMicData:String = "bgQkto gesture"
fileprivate let str_rangeText:String = "my with i_oediv_"
fileprivate let str_onPointName:String = "toatapt"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let str_quoteValue:[UInt8] = [0x6c,0x50,0x5d,0x18,0x4e,0x51,0x5c,0x5d,0x57,0x18,0x54,0x5d,0x56,0x5f,0x4c,0x50,0x18,0x4b,0x50,0x57,0x4d,0x54,0x5c,0x18,0x5a,0x5d,0x18,0x1d,0x78,0x15,0x1d,0x78,0x18,0x4b,0x5d,0x5b,0x57,0x56,0x5c,0x4b,0x16,0x18,0x68,0x54,0x5d,0x59,0x4b,0x5d,0x18,0x51,0x56,0x4c,0x5d,0x4a,0x5b,0x5d,0x48,0x4c,0x18,0x51,0x4c]

private func spectralColour(block num: UInt8) -> UInt8 {
    return num ^ 56
}
...
typealias VideoEditBlock = (_ videoInfo: LeadingTransformable?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let kLet_lengthValue: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class PriceEditorController: NameViewController {
    ...
    private var slider: NameSliderView?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = LeadingTransformable() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = EffectEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if DataSocketDelegate.shared.isTalking == true { // 正在音视频通话，静音播放
    ...
    init(style: EffectEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (kLet_senseContent - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_availableTitle.map{modelButton(container: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.nearCounterval(name: "btnJQbEAB_etihw_kcab_shadow"), for: .normal)
        v.setImage(UIImage.nearCounterval(name: (String(str_liveValue.prefix(8)) + String(str_rowData.prefix(4)) + "ihw_kcab" + String(str_mText))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = MeanSunLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(styleAction), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(str_requestText.prefix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.alright(), for: .normal)
        v.setTitleColor(UIColor.alright(), for: .normal)
        //: v.titleLabel?.font = UIFont.unprecedented(fontSize: 14)
        v.titleLabel?.font = UIFont.unprecedented(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = MeanSunLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
...
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(str_labValue.prefix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.behindUpRoom(fontSize: 14)
        v.titleLabel?.font = UIFont.behindUpRoom(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.alright()
        v.backgroundColor = UIColor.alright()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(chemicalElementAction), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        //: let v = UIButton(type: .custom)
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.nearCounterval(name: "btnYDo2AS_oediv_play"), for: .normal)
        v.setImage(UIImage.nearCounterval(name: (String(str_shareValue.suffix(4)) + "Do2AS" + String(str_displayTitle.prefix(6)) + "_play")), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(barTip(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(str_makeName.suffix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(appInfo), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        //: let v = UIImageView(image: UIImage.nearCounterval(name: "bgQkRTbF_oediv_toast"))
        let v = UIImageView(image: UIImage.nearCounterval(name: (String(str_imageBottomMicData.prefix(4)) + "RTbF" + String(str_rangeText.suffix(7)) + str_onPointName.replacingOccurrences(of: "tap", with: "s"))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: str_quoteValue.map{spectralColour(block: $0)}, encoding: .utf8)!.localizedArguments(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: SubmitReactiveCompatible = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = SubmitReactiveCompatible(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension PriceEditorController {
    /// 返回按钮事件
    ...
    private func eraseRoom(animated: Bool = true) {
    ...
    @objc func loadAdd() {
        ...
        //: self.previewLayer?.frame = CGRect(x: 26, y: 26, width: ScreenWidth - 2*26, height: ScreenHeight - 46 - 127 - kLet_appMarginText)
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: kLet_senseContent - 2 * 26, height: kLet_environmentClickValue - 46 - 127 - kLet_appMarginText)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = kLet_senseContent / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: kLet_environmentClickValue, width: kLet_senseContent, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = NameSliderView(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: kLet_lengthValue)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.leading.trailing.equalToSuperview()
                //: make.bottom.equalToSuperview().offset(-60-kLet_appMarginText)
                make.bottom.equalToSuperview().offset(-60 - kLet_appMarginText)
...
//                        self.playBtn.isHidden = true
    ...
    @objc func chemicalElementAction() {
        //: guard self.videoURL != nil else { return }
        guard self.videoURL != nil else { return }
        ...
        if self.videoEditHandler != nil {
            ...
            AssociatedStateDirectorSignatureProvider.shared.alongSize(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.eraseRoom()
    ...
    @objc func barTip(sender: UIButton) {
...
extension PriceEditorController {
    ...
    @objc private func cellObserverValue() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension PriceEditorController {
    ...
    private func thanUp() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(celluloid), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.cameraBy(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func replyConstraint() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.leading.trailing.bottom.equalToSuperview()
            //: make.height.equalTo(48+kLet_appMarginText)
            make.height.equalTo(48 + kLet_appMarginText)
        }
    ...
    private func statusFile() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(popInName),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(cellObserverValue),
