...
fileprivate let str_interactionValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "FFD855" :*/
fileprivate let str_attributeValue:[Character] = ["F","F","D","8","5","5"]
...
//: let LeftMargin = 43.0
let kLet_succeedDailyOutContent = 43.0
//: let kSliderWidth = 20.0
let kLet_tabValue = 20.0
...
class PopEmptyView: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - kLet_tabValue
                ...
                self.sound(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_interactionValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (String(str_attributeValue)))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: kLet_succeedDailyOutContent, y: 0, width: kLet_tabValue, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - kLet_tabValue, y: 0, width: kLet_tabValue, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.equalRgba(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: kLet_succeedDailyOutContent, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.equalRgba(0, 0, 0, 0.6)
...
extension PopEmptyView {
    ...
    func sound(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - kLet_tabValue,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kLet_tabValue)
    }
}
...
extension PopEmptyView {
    ...
    @objc func cellGesture(gesture: UIPanGestureRecognizer) {
    ...
    @objc func slideModelMotion(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension PopEmptyView {
    ...
    private func indexColor() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(cellGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(slideModelMotion(gesture:)))
    ...
    func cameraBy(_ xposition: CGFloat) {
