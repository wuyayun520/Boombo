...
fileprivate let str_showBarImageContent:[UInt8] = [0xa7,0xa0,0xa7,0xba,0xe6,0xad,0xa1,0xaa,0xab,0xbc,0xf4,0xe7,0xee,0xa6,0xaf,0xbd,0xee,0xa0,0xa1,0xba,0xee,0xac,0xab,0xab,0xa0,0xee,0xa7,0xa3,0xbe,0xa2,0xab,0xa3,0xab,0xa0,0xba,0xab,0xaa]

private func messageSucceed(player num: UInt8) -> UInt8 {
    return num ^ 206
}
...
class NameSliderView: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (kLet_succeedDailyOutContent - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_showBarImageContent.map{messageSucceed(player: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.upPoint(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: PopEmptyView = {
        ...
        let overlay = PopEmptyView(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension NameSliderView {
    ...
    private func upPoint(point: CGFloat) -> UIImage {
...
extension NameSliderView: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = kLet_succeedDailyOutContent - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.imageDirection(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.imageDirection(isShow: true)
    }
...
extension NameSliderView {
    ...
    private func startModel() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(kLet_succeedDailyOutContent)
