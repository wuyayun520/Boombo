...
/*: / :*/
fileprivate let str_timeValue:[Character] = ["/"]

/*: "\n" :*/
fileprivate let str_labValue:[Character] = ["\n"]
...
struct FromMessageContainerInset {
    //: let top: CGFloat = 5.0
    let top: CGFloat = 5.0
...
class LyricView: UIView {
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        tipBy(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        tipBy(placeholder: nil, placeholderColor: nil)
    }
}
...
extension LyricView {
    ...
    private func tipBy(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(EffectLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(EffectLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-EffectLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: FromMessageContainerInset().top, left: FromMessageContainerInset().left, bottom: FromMessageContainerInset().bottom, right: FromMessageContainerInset().right)
...
extension LyricView: UITextViewDelegate {
