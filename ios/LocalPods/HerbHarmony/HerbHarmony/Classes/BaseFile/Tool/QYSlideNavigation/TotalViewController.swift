...
protocol AnglicismNavigationDelegate: AnyObject {
...
class TotalViewController: UIViewController {
    ...
    weak var delegate: AnglicismNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = IconThen.getScreenHeight() - (topDistance + kTitle_H)
        var height = IconThen.deleteGift() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= kLet_callValue
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: IconThen.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: IconThen.atWidth(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(IconThen.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(IconThen.atWidth()), height: 0)
        ...
        scrollView.semanticContentAttribute = MeanSunLanguageManager.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: IconThen.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: IconThen.atWidth()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: IconThen.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: IconThen.atWidth()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = MeanSunLanguageManager.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(fastener), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = IconThen.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = IconThen.atWidth() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(IconThen.getScreenWidth()-originX-titleWidth)
            let x = MeanSunLanguageManager.shared.direction == .leftToRight ? originX:(IconThen.atWidth()-originX-titleWidth)
...
extension TotalViewController {
    ...
    @objc private func fastener(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        headingEventGift(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * IconThen.getScreenWidth()
        let offset_x = CGFloat(index) * IconThen.atWidth()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        loadModel(withIndex: index)
        //: seletedViewController(withIndex: index)
        priority(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func loadModel(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * IconThen.getScreenWidth()
        let origin_x = CGFloat(index) * IconThen.atWidth()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: IconThen.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: IconThen.atWidth(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func headingEventGift(_ button: UIButton) {
    ...
    func superiorCenter(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= IconThen.getScreenWidth() {
        if titleScrollView.contentSize.width <= IconThen.atWidth() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - IconThen.getScreenWidth() * 0.5
        var offset_x = sender.center.x - IconThen.atWidth() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - IconThen.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - IconThen.atWidth()
...
extension TotalViewController: UIScrollViewDelegate {
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / IconThen.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / IconThen.atWidth())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            headingEventGift(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        loadModel(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / IconThen.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / IconThen.atWidth())
        ...
        //: let scaleR = scrollView.contentOffset.x / IconThen.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / IconThen.atWidth() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
