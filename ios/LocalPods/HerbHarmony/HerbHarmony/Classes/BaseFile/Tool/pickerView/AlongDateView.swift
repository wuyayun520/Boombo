...
/*: "Select a date" :*/
fileprivate let str_colorShareText:[Character] = ["S","e","l","e","c","t"," "]
fileprivate let str_pathEqualData:String = "white texta date"

/*: M" :*/
fileprivate let str_frameValue:String = "data"

/*: D" :*/
fileprivate let str_textValue:String = "fail"

/*: Y" :*/
fileprivate let str_statusName:String = "of"
...
protocol SolarDayPopStreetwiseObjectProtocol: NSObjectProtocol {
...
class AlongDateView: PricePickView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: SolarDayPopStreetwiseObjectProtocol?
    ...
    override func touchName() {
        ...
        titleString = (String(str_colorShareText) + String(str_pathEqualData.suffix(6))).localized
        ...
        imageDay(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func imageDay(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func listenerTime(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.area(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.area(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return drunkTitle(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func drunkTitle(year: Int, month: Int) -> Int {
