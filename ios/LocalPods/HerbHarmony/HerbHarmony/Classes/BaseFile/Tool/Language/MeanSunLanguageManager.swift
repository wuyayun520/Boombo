...
/*: lproj" :*/
fileprivate let str_playerName:String = "lpinputj"

/*: "lproj" :*/
fileprivate let str_lineGiftData:String = "lprleadingj"
...
@objcMembers open class MeanSunLanguageManager: NSObject {
    //: public static let shared = LanguageManager()
    public static let shared = MeanSunLanguageManager()
    //: private(set) var currLanguage: String = IconThen.getSystemLangCode()
    private(set) var currLanguage: String = IconThen.beforeNameGift()
...
extension String {
    ...
    private var languageBundle: Bundle {
        ...
        guard let languageFolder = ViewPodspecBundle.bundle.path(forResource: "\(kLet_recordValue)" + (str_playerName.replacingOccurrences(of: "input", with: "ro")), ofType: nil),
              //: let languageBundle = Bundle(path: languageFolder)?.path(forResource: language, ofType: "lproj"),
              let languageBundle = Bundle(path: languageFolder)?.path(forResource: language, ofType: (str_lineGiftData.replacingOccurrences(of: "leading", with: "o"))),
