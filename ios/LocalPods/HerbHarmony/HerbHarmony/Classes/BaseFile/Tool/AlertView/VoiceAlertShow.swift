...
fileprivate let str_labelRoomValue:[Character] = ["b","t","n","_","s","h","o","w","_","a","l","e","r","t","B","a","c"]
fileprivate let str_userValue:[Character] = ["k"]
fileprivate let str_conversationData:String = "equalound"

/*: "btn_alertBackground_false" :*/
fileprivate let str_styleColorLetData:String = "gift event height color rawbtn_al"
fileprivate let str_bottomData:String = "ackgroumain"
fileprivate let str_resumeLocalTitle:String = "d_falseheight user true push"
...
public class VoiceAlertShow: NSObject {
    /// 通用回调
...
public extension VoiceAlertShow {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: ManagerAlertConfig) -> Void
    ...
    internal class func enablelineMedium(title: String? = nil,
                                         //: message: String?  = nil,
                                         message: String? = nil,
                                         ...
                                         config: ManagerAlertConfig? = nil)
    {
        //: customAlert(title: title,
        customForTotal(title: title,
                       //: message: message,
                       message: message,
    ...
    @objc internal class func migration(title: String? = nil,
    ...
    class func customForTotal(title: String? = nil,
                              ...
                              config: ManagerAlertConfig? = nil)
    {
        ...
        let alertView = TouchReactiveCompatible(title: title,
        ...
        //: IconThen.getWindow().addSubview(alertView)
        IconThen.getBind().addSubview(alertView)
    ...
    class func bias() {
        //: IconThen.getWindow().subviews.forEach { (view) in
        for view in IconThen.getBind().subviews {
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: TouchReactiveCompatible.self) {
    ...
    class func dismissTouch(title: String, isTouch: Bool) {
        //: IconThen.getWindow().subviews.forEach { (view) in
        for view in IconThen.getBind().subviews {
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: TouchReactiveCompatible.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: TouchReactiveCompatible = view as! TouchReactiveCompatible
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.addTouch(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class ManagerAlertConfig {
    /// 执行动画时间
    ...
    public var maskType: StartMaskType = .color
    /// 阴影
    ...
    /// alert标题字体
    //: public var titleFont: UIFont = .liveAttention(type: .Medium, fontSize: 18)
    public var titleFont: UIFont = .liveAttention(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.occurrence()
    public var titleColor = UIColor.occurrence()
    /// alert信息字体
    //: public var textFont: UIFont = .liveAttention(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .liveAttention(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.occurrence()
    public var textColor = UIColor.occurrence()
    //: public var textlineBreakMode = NSLineBreakMode.byWordWrapping
    public var textlineBreakMode = NSLineBreakMode.byWordWrapping

    /// alert按钮字体
    //: public var buttonFont: UIFont = .liveAttention(type: .Medium, fontSize: 16)
    public var buttonFont: UIFont = .liveAttention(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.alright()
    public var leftColor = UIColor.alright()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(str_labelRoomValue) + String(str_userValue) + str_conversationData.replacingOccurrences(of: "equal", with: "gr"))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(str_styleColorLetData.suffix(6)) + "ertB" + str_bottomData.replacingOccurrences(of: "main", with: "n") + String(str_resumeLocalTitle.prefix(7)))
    //: public var rightFont: UIFont = .liveAttention(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .liveAttention(type: .Medium, fontSize: 16)
