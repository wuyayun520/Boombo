...
fileprivate let str_toValue:[UInt8] = [0xac,0xab,0xac,0xb1,0xed,0xa6,0xaa,0xa1,0xa0,0xb7,0xff,0xec,0xe5,0xad,0xa4,0xb6,0xe5,0xab,0xaa,0xb1,0xe5,0xa7,0xa0,0xa0,0xab,0xe5,0xac,0xa8,0xb5,0xa9,0xa0,0xa8,0xa0,0xab,0xb1,0xa0,0xa1]

private func progressBiasApp(max num: UInt8) -> UInt8 {
    return num ^ 197
}

/*: "8075F5" :*/
fileprivate let str_awakeViewName:String = "color any activity target on8075F5"

/*: "9A9B9C" :*/
fileprivate let str_mysteryText:String = "value"
fileprivate let str_addTitle:String = "actual9B9C"

/*: "kOpacityAnimation" :*/
fileprivate let str_cameraData:[Character] = ["k","O","p","a","c","i","t","y"]
fileprivate let str_centerContent:[Character] = ["A","n","i","m","a","t","i","o","n"]

/*: "opacity" :*/
fileprivate let str_infoName:[Character] = ["o","p","a","c","i","t"]
fileprivate let str_insideText:String = "style"

/*: "forwards" :*/
fileprivate let str_viewValue:String = "forwardatas"

/*: "easeIn" :*/
fileprivate let str_selectedValue:[Character] = ["e","a","s","e","I","n"]

/*: "8C7AF8" :*/
fileprivate let str_toText:[Character] = ["8"]
fileprivate let str_appWithName:[Character] = ["C","7","A","F","8"]
...
class BindInputView: UIView {
    ...
    public init(frame: CGRect, codeNumber: Int = 6, labelTextColor: UIColor = UIColor.black, mainColor: UIColor = UIColor.orange, normalColor: UIColor = UIColor.gray, margin: CGFloat = 12.0) {
        ...
        self.bottomSumeraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: str_toValue.map{progressBiasApp(max: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(callBy(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension BindInputView {
    ...
    func line() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(str_awakeViewName.suffix(6))))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (str_mysteryText.replacingOccurrences(of: "value", with: "9") + str_addTitle.replacingOccurrences(of: "actual", with: "A")))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func currentHidden(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(str_cameraData) + String(str_centerContent)))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(toAnimation(), forKey: (String(str_cameraData) + String(str_centerContent)))
        }
    ...
    fileprivate func toAnimation() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (String(str_infoName) + str_insideText.replacingOccurrences(of: "style", with: "y")))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (str_viewValue.replacingOccurrences(of: "data", with: "d")))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(str_selectedValue))))
        //: return animation
        return animation
    }
}
...
extension BindInputView {
    ...
    private func go() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(str_toText) + String(str_appWithName)))?.cgColor
    ...
    private func bottomSumeraction() {}
}
