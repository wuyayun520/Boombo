...
fileprivate let str_keyLeadingValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_live_toast_bg" :*/
fileprivate let str_whiteData:String = "btn_lcover view"
fileprivate let str_directionText:String = "self in icon_bg"

/*: "btn_live_toast_arrow" :*/
fileprivate let str_assetTitle:String = "B"
fileprivate let str_faceContextTitle:[Character] = ["t","n","_","l","i","v","e","_","t","o","a","s","t","_","a","r","r","o","w"]
...
var kLet_buttonText = false

//: class TalkingLiveTabToast: UIView {
class InfoTabToast: UIView {
    // 气泡间距
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_keyLeadingValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bubbleImgView: UIImageView = {
        ...
        let imgV = UIImageView()
        //: var img = UIImage.nearCounterval(name: "btn_live_toast_bg").resizableImage(withCapInsets: bubbleInsets, resizingMode: .stretch)
        var img = UIImage.nearCounterval(name: (String(str_whiteData.prefix(5)) + "ive_toast" + String(str_directionText.suffix(3)))).resizableImage(withCapInsets: bubbleInsets, resizingMode: .stretch)
        //: imgV.image = img
        imgV.image = img
        //: return imgV
        return imgV
    ...
    private lazy var arrowImgView: UIImageView = {
        //: let imgV = UIImageView()
        let imgV = UIImageView()
        //: imgV.image = UIImage.nearCounterval(name: "btn_live_toast_arrow")
        imgV.image = UIImage.nearCounterval(name: (str_assetTitle.lowercased() + String(str_faceContextTitle)))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension InfoTabToast {
    /// 展示视图，5s后自动移除
    //: func show(superView: UIView) {
    func spectaclesError(superView: UIView) {
        //: guard LIVETABTOASTISSHOW == false else { return }
        guard kLet_buttonText == false else { return }
        ...
        //: guard WithAdministratorAppManager.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        guard WithAdministratorAppManager.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        //: LIVETABTOASTISSHOW = true
        kLet_buttonText = true

        //: textLab.text = WithAdministratorAppManager.share.appUserConfigMode.liveTabTips
        textLab.text = WithAdministratorAppManager.share.appUserConfigMode.liveTabTips
        //: let maxWidth = ScreenWidth - 48
        let maxWidth = kLet_senseContent - 48
