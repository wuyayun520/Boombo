...
fileprivate let str_succeedName:[UInt8] = [0xf6,0xf1,0xf6,0xeb,0xb7,0xfc,0xf0,0xfb,0xfa,0xed,0xa5,0xb6,0xbf,0xf7,0xfe,0xec,0xbf,0xf1,0xf0,0xeb,0xbf,0xfd,0xfa,0xfa,0xf1,0xbf,0xf6,0xf2,0xef,0xf3,0xfa,0xf2,0xfa,0xf1,0xeb,0xfa,0xfb]

private func atView(m num: UInt8) -> UInt8 {
    return num ^ 159
}
...
class ScaleReactiveCompatible: NameViewController {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: CounteractionTransitioningDelegate!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = CounteractionTransitioningDelegate(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: str_succeedName.map{atView(m: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class CounteractionTransitioningDelegate: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: kLet_senseContent, height: kLet_environmentClickValue * 0.6)
    ...
    func atMakeRadius(corners: UIRectCorner, radius: CGFloat) {
...
extension CounteractionTransitioningDelegate: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = CoattailsEffectPresentationController(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension CounteractionTransitioningDelegate: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            wokContext(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            someDestroy(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func wokContext(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func someDestroy(transitionContext: UIViewControllerContextTransitioning) {
...
class CoattailsEffectPresentationController: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: CounteractionTransitioningDelegate!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: CounteractionTransitioningDelegate, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func addStatus() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(glossy))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: kLet_senseContent, height: kLet_environmentClickValue)
