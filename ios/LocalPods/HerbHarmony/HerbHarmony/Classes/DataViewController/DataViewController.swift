...
class DataViewController: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: SubmitNumeric?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.detoxify()
            }
        }
    ...
    func navibarAppearanceSetup() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.errorAtImage(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.air()
            //: barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.occurrence(), NSAttributedString.Key.font: UIFont.liveAttention(type: .Medium, fontSize: 17)]
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.occurrence(), NSAttributedString.Key.font: UIFont.liveAttention(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.errorAtImage(color: UIColor.white), for: .any, barMetrics: .default)
            //: navigationBar.shadowImage = UIImage.init()
            navigationBar.shadowImage = UIImage.init()
            //: navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.occurrence(), NSAttributedString.Key.font: UIFont.liveAttention(type: .Medium, fontSize: 17)]
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.occurrence(), NSAttributedString.Key.font: UIFont.liveAttention(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.occurrence()
        UINavigationBar.appearance().tintColor = UIColor.occurrence()
...
extension DataViewController: UIGestureRecognizerDelegate {
