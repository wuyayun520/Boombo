...
/*: "&sr=6" :*/
fileprivate let str_squareName:[Character] = ["&","s","r","=","6"]
...
enum ModelVolumeTableConvertible: String, CaseIterable {
...
class BarbManager: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func subPurchaseArray(urlType: MBridgeCollection) -> String {
        //: var url = WithAdministratorAppManager.share.appConfigMode.urlH5Domain
        var url = WithAdministratorAppManager.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(WithAdministratorAppManager.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : WithAdministratorAppManager.share.appUserConfigMode.fullRechargePage)
            url.append(WithAdministratorAppManager.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : WithAdministratorAppManager.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(WithAdministratorAppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : WithAdministratorAppManager.share.appUserConfigMode.halfRechargePage)
            url.append(WithAdministratorAppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : WithAdministratorAppManager.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = WithAdministratorAppManager.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = WithAdministratorAppManager.share.appUserConfigMode.halfRechargePage.appending((String(str_squareName)))
            //: url.append(WithAdministratorAppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(WithAdministratorAppManager.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
