...
/*: "id" :*/
fileprivate let str_dataValue:String = "iinfo"
...
struct ClimaxModelType: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum StackClusterLiteral: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct ExtraMeasurable: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: QueryHashable?
}

///
//: enum MomentVideoStatus: String {
enum QueryHashable: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension ModelHandyJSON {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func collection(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            //: textHeight = Int(TalkingMomentTextSizeView.caculateSizeWithText(text: content!, size: CGSize.init(width: ScreenWidth-54, height: CGFloat(MAXFLOAT)), font: UIFont.liveAttention(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber==0 ? 20:0))
            textHeight = Int(ButtonBackSizeView.level(text: content!, size: CGSize(width: kLet_senseContent - 54, height: CGFloat(MAXFLOAT)), font: UIFont.liveAttention(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == StackClusterLiteral.Photo.rawValue || momentType == StackClusterLiteral.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
