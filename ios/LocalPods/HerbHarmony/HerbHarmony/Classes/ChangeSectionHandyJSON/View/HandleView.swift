...
fileprivate let str_equalValue:[UInt8] = [0xb6,0xb1,0xb6,0xab,0xf7,0xbc,0xb0,0xbb,0xba,0xad,0xe5,0xf6,0xff,0xb7,0xbe,0xac,0xff,0xb1,0xb0,0xab,0xff,0xbd,0xba,0xba,0xb1,0xff,0xb6,0xb2,0xaf,0xb3,0xba,0xb2,0xba,0xb1,0xab,0xba,0xbb]

private func elseLeft(current num: UInt8) -> UInt8 {
    return num ^ 223
}

/*: "btn_dynamic_stop_nor" :*/
fileprivate let str_wrapName:[Character] = ["b","t","n","_","d","y"]
fileprivate let str_nameUpValue:[Character] = ["n","a","m","i","c","_","s","t","o","p","_","n","o","r"]

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let str_giftTitle:[Character] = ["b","t","n","w","O","b","l","E","h","_","t","d","j","_","o"]
fileprivate let str_mainValue:String = "ediv_stopsuper dismiss room"

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let str_readingSucceedPhoneData:String = "live let file addbtnCm"
fileprivate let str_errorData:[Character] = ["j","_","o","e","d","i","v","_","p","l","a","y"]

/*: "%@" :*/
fileprivate let str_modelValue:String = "%attach"
...
protocol ModelThen: NSObject {
    //: func func__interactionViewStatus(view: TalkingVideoPlayerInteractionView, status: InteractionViewStatus)
    func subordinate(view: HandleView, status: PopManagerViewStatus)

    //: func func__interactionViewSlideValue(view: TalkingVideoPlayerInteractionView, value: CGFloat)
    func along(view: HandleView, value: CGFloat)
}

//: class TalkingVideoPlayerInteractionView: UIView {
class HandleView: UIView {
    //: open weak var delegate: InteractionViewDelegate?
    open weak var delegate: ModelThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_equalValue.map{elseLeft(current: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setImage(UIImage.nearCounterval(name: "btn_dynamic_stop_nor"), for: .normal)
        btn.setImage(UIImage.nearCounterval(name: (String(str_wrapName) + String(str_nameUpValue))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(qualitySender(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setImage(UIImage.nearCounterval(name: "btnwOblEh_tdj_oediv_stop"), for: .normal)
        btn.setImage(UIImage.nearCounterval(name: (String(str_giftTitle) + String(str_mainValue.prefix(9)))), for: .normal)
        //: btn.setImage(UIImage.nearCounterval(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.nearCounterval(name: (String(str_readingSucceedPhoneData.suffix(5)) + "joS7_td" + String(str_errorData))), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(playWithinDoinge(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var failImageView: UIImageView = {
        //: let imag = UIImageView()
        let imag = UIImageView()
        //: imag.image = UIImage.nearCounterval(name: "")
        imag.image = UIImage.nearCounterval(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: InstrumentalityPlayerSlider = {
        ...
        let slid = InstrumentalityPlayerSlider(frame: CGRect(x: 100, y: 0, width: kLet_senseContent - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension HandleView {
    ...
    private func labInput() {
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.along(view: self, value: value)
        }
