...
fileprivate let str_succeedToValue:[UInt8] = [0x1c,0x1b,0x1c,0x1,0x5d,0x16,0x1a,0x11,0x10,0x7,0x4f,0x5c,0x55,0x1d,0x14,0x6,0x55,0x1b,0x1a,0x1,0x55,0x17,0x10,0x10,0x1b,0x55,0x1c,0x18,0x5,0x19,0x10,0x18,0x10,0x1b,0x1,0x10,0x11]

/*: "#4FAAFF" :*/
fileprivate let str_pointName:[Character] = ["#","4","F","A","A","F"]
fileprivate let str_equalData:[Character] = ["F"]

/*: "reply" :*/
fileprivate let str_actualText:[Character] = ["r","e","p","l","y"]

/*: "： :*/
fileprivate let str_backgroundValue:[Character] = ["\u{ff1a}"]

/*: "F5F5F5" :*/
fileprivate let str_makeValue:String = "Ffromfrom5"
...
class ConcentrationView: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_succeedToValue.map{$0^117}, encoding: .utf8)!)
    }
...
extension ConcentrationView {
    ...
    @objc func each(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension ConcentrationView {
    //: func configCell(model: TalkingCommentReplyModel) {
    func hypothesisMake(model: ChangeSectionHandyJSON) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(str_pointName) + String(str_equalData)))
            ...
            //: var fontA = [UIFont]()
            var fontA = [UIFont]()
            //: let font = UIFont.unprecedented(fontSize: 14)
            let font = UIFont.unprecedented(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (String(str_actualText)).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.inexpensiveness(), UIColor.occurrence(), UIColor.inexpensiveness(), UIColor.occurrence()]
                colorA = [UIColor.inexpensiveness(), UIColor.occurrence(), UIColor.inexpensiveness(), UIColor.occurrence()]
                ...
                strA = [model.nickname, "：\(model.content)"]
                //: colorA = [UIColor.inexpensiveness(), UIColor.occurrence()]
                colorA = [UIColor.inexpensiveness(), UIColor.occurrence()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func imageShould(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: kLet_senseContent - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (str_makeValue.replacingOccurrences(of: "from", with: "5F")))?.cgColor
...
extension ConcentrationView {
    ...
    private func viewStart() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(each(sender:)))
