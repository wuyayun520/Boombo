...
/*: "%@%@" :*/
fileprivate let str_currentName:[Character] = ["%","@","%","@"]

/*: "/TXCache" :*/
fileprivate let str_viewData:String = "color bind/T"
fileprivate let str_imageName:[Character] = ["X","C","a","c","h","e"]
...
protocol MainReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func poolPlayerReport(player: ButtonPlayerManager, status: TotalPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func informant(player: ButtonPlayerManager, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func showSub(player: ButtonPlayerManager, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class ButtonPlayerManager: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: MainReactiveCompatible?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: TotalPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = ButtonPlayerManager()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(str_viewData.suffix(2)) + String(str_imageName))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(popSection), userInfo: nil, repeats: true)
...
extension ButtonPlayerManager {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func drunk(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: kLet_clickValue)
    ...
    @objc func popSection() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.showSub(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func last(renderMode: ConditionRenderMode) {
    ...
    func exampleSize(time: Float) {
        //: self.player.seek(time)
        self.player.seek(time)
    }

    //: func height() ->Int {
    func toBackground() -> Int {
    ...
    func manager(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isTalking == true {
        if DataSocketDelegate.shared.isTalking == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func behindButton() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.poolPlayerReport(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func keyFrontPlayings() -> Bool {
...
extension ButtonPlayerManager: TXVodPlayListener {
    ...
    func onPlayEvent(_ player: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.informant(player: self, duration: Int(duration), currentTime: Int(currentTime))
