...
/*: "tab" :*/
fileprivate let str_customValue:[Character] = ["t","a","b"]

/*: "page" :*/
fileprivate let str_contentData:[Character] = ["p","a","g","e"]

/*: "uid" :*/
fileprivate let str_modelValue:String = "uice"

/*: "model" :*/
fileprivate let str_userName:String = "todel"

/*: "ItemToThen" :*/
fileprivate let str_valueData:[Character] = ["T","a","l","k","i","n","g","M","o","m"]
fileprivate let str_callTitle:[Character] = ["e","n","t","I","t","e","m","C","e","l","l"]

/*: "There's no feed yet." :*/
fileprivate let str_textTitle:String = "Therenote male make empty count"
fileprivate let str_toText:String = "feed pile lab"
...
class SumryMomentVc: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: kLet_callValue, right: 0)
        //: return table
        return table
...
extension SumryMomentVc {
    //: func reqData() {
    func fillData() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(String(str_customValue))] = 1
        //: dict["page"] = pageIndex
        dict[(String(str_contentData))] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        ManagerRequestTool.parShow(params: dict) { succeed, result, errorModel in
...
extension SumryMomentVc {
    ...
    @objc func heliogram(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(str_modelValue.replacingOccurrences(of: "ice", with: "id"))] as! String
    ...
    @objc func intervalerval(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: ModelHandyJSON = userinfo[(str_userName.replacingOccurrences(of: "to", with: "mo"))] as! ModelHandyJSON
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func timeBy(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: ModelHandyJSON = userinfo[(str_userName.replacingOccurrences(of: "to", with: "mo"))] as! ModelHandyJSON
...
extension SumryMomentVc: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = (String(str_valueData) + String(str_callTitle))
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? ItemToThen
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = ItemToThen(style: .default, reuseIdentifier: identifier)
        }
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == StackClusterLiteral.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: ItemToThen = cell as? ItemToThen else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
                    tempcell.equidistant()
                }
            }
        }
    }
...
extension SumryMomentVc: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        mediumView(scrollView: scrollView)
    }
    ...
    func mediumView(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: ItemToThen = mainTableView.cellForRow(at: middleIndexPath) as! ItemToThen
...
// MARK: - IconObjectProtocol

//: extension TalkingPopularMomentVC {
extension SumryMomentVc {
    //: @objc func PostSuccess() {
    @objc func postGodspeed() {
        //: mainTableView.mj_header?.beginRefreshing()
        mainTableView.mj_header?.beginRefreshing()
    }
}
...
extension SumryMomentVc {
    ...
    private func setupCurrent() {
        ...
        style.TipsTitle = (String(str_textTitle.prefix(5)) + "\'s no " + String(str_toText.prefix(5)) + "yet.").localized
        ...
        mainTableView.register(ItemToThen.self, forCellReuseIdentifier: (String(str_valueData) + String(str_callTitle)))
    ...
    private func constraintImage() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(intervalerval(notification:)), name: kLet_scalePlainData, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(heliogram(notification:)), name: kLet_lineValue, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(timeBy(notification:)), name: kLet_pageTitle, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(postGodspeed), name: kLet_micText, object: nil)
    }
}
