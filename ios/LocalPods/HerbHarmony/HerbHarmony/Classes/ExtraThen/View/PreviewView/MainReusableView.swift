...
fileprivate let str_conversationTitle:[UInt8] = [0x64,0x63,0x64,0x79,0x25,0x6e,0x62,0x69,0x68,0x7f,0x37,0x24,0x2d,0x65,0x6c,0x7e,0x2d,0x63,0x62,0x79,0x2d,0x6f,0x68,0x68,0x63,0x2d,0x64,0x60,0x7d,0x61,0x68,0x60,0x68,0x63,0x79,0x68,0x69]

private func resultVertical(view num: UInt8) -> UInt8 {
    return num ^ 13
}

/*: "img_loading_lose" :*/
fileprivate let str_imageValue:[Character] = ["i","m","g","_","l","o","a","d","i","n"]
fileprivate let str_borderName:[Character] = ["g","_","l","o","s","e"]
...
class MainReusableView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_conversationTitle.map{resultVertical(view: $0)}, encoding: .utf8)!)
    }
...
extension MainReusableView {
    //: private func bindInteraction() {
    private func nameMake() {
        //: let singleTap = UITapGestureRecognizer(target: self, action: #selector(singleTapAction))
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(subAction))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(content)).then {
...
extension MainReusableView {
    ...
    func hypothesisRemove(imgModel: PossibilityHandyJSON) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(str_imageValue) + String(str_borderName))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.regainMakeSubviews()
            }
        }
    ...
    func afterBind(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(str_imageValue) + String(str_borderName))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.regainMakeSubviews()
        }
    }
...
extension MainReusableView: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension MainReusableView {
    ...
    func regainMakeSubviews() {
        ...
        var newW = kLet_senseContent
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = kLet_senseContent * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > kLet_environmentClickValue {
            //: newH = ScreenHeight
            newH = kLet_environmentClickValue
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * kLet_environmentClickValue / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (kLet_senseContent - newW) / 2.0, y: (kLet_environmentClickValue - newH) / 2)
