...
/*: "#AAAAAA" :*/
fileprivate let str_appTitle:String = "for self receive video white#AAAAA"
fileprivate let str_loadTitle:[Character] = ["A"]

/*: " No more than 10 characters" :*/
fileprivate let str_currentValue:String = "bug if size No mo"
fileprivate let str_equalText:String = "an 10else let memory bar let"
fileprivate let str_tableValue:[Character] = ["e","r","s"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_bottomText:[UInt8] = [0xa6,0xa1,0xa6,0xbb,0xe7,0xac,0xa0,0xab,0xaa,0xbd,0xf5,0xe6,0xef,0xa7,0xae,0xbc,0xef,0xa1,0xa0,0xbb,0xef,0xad,0xaa,0xaa,0xa1,0xef,0xa6,0xa2,0xbf,0xa3,0xaa,0xa2,0xaa,0xa1,0xbb,0xaa,0xab]

private func fullPhaseOfTheMoon(value num: UInt8) -> UInt8 {
    return num ^ 207
}
...
class MiniDataViewDelegate: UIView {
    //: private var alertConfig: ShowAlertConfig
    private var alertConfig: ManagerAlertConfig
    ...
    private lazy var inputTView: MediaThen = {
        //: let input = InputTextView()
        let input = MediaThen()
        //: input.font = UIFont.liveAttention(type: .Regular, fontSize: 16)
        input.font = UIFont.liveAttention(type: .Regular, fontSize: 16)
        //: input.placeholderColor = .init(hex: "#AAAAAA") ?? UIColor.gray
        input.placeholderColor = .init(hex: (String(str_appTitle.suffix(6)) + String(str_loadTitle))) ?? UIColor.gray
        //: input.placeholder = " No more than 10 characters".localized
        input.placeholder = (String(str_currentValue.suffix(6)) + "re th" + String(str_equalText.prefix(5)) + " charact" + String(str_tableValue)).localized
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.cornerRadius = alertConfig.BtncornerRadius
        //: leftBtn.layer.borderColor = UIColor.alright().cgColor
        leftBtn.layer.borderColor = UIColor.alright().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(bear), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        rightBtn.setBackgroundImage(UIImage.nameColors(colors: UIColor.modelColor(), size: CGSize(width: 267, height: 50)), for: .normal)
        ...
        rightBtn.setBackgroundImage(UIImage.nameColors(colors: UIColor.fromPath(), size: CGSize(width: 267, height: 50)), for: .disabled)
        ...
        rightBtn.addTarget(self, action: #selector(coin), for: .touchUpInside)
    ...
    init(title: String?,
         ...
         config: ManagerAlertConfig?)
    {
        //: let model = config ?? ShowAlertConfig.init()
        let model = config ?? ManagerAlertConfig()
        //: alertConfig = model
        alertConfig = model
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_bottomText.map{fullPhaseOfTheMoon(value: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func putOpen(title: String, isTouch: Bool) {
        ...
        //: if !isTouch {
        if !isTouch {
            //: rightBtn.setBackgroundImage(UIImage.nearCounterval(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            rightBtn.setBackgroundImage(UIImage.nearCounterval(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.nearCounterval(name: alertConfig.rightBackgroundImage).withTintColor(.alright())
            let image = UIImage.nearCounterval(name: alertConfig.rightBackgroundImage).withTintColor(.alright())
...
extension MiniDataViewDelegate: UITextViewDelegate {
    //: func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    func grainfieldInstance(_ textField: UITextField, shouldChangeCharactersIn _: NSRange, replacementString string: String) -> Bool {
    ...
    func letterNextReturn(_ textField: UITextField) -> Bool {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > alertConfig.lengthLimit {
            ...
            if textNum > alertConfig.lengthLimit && alertConfig.lengthLimit > 0 {
                ...
                textView.text = effectTitle(index: alertConfig.lengthLimit, text: textContent)
            }
        }
    }

    //: private func string_prefix(index: Int, text: String) -> String {
    private func effectTitle(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
