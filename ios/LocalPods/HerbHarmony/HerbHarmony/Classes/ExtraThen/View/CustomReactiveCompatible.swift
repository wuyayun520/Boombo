...
fileprivate let str_titleName:[UInt8] = [0x7e,0x79,0x7e,0x63,0x3f,0x74,0x78,0x73,0x72,0x65,0x2d,0x3e,0x37,0x7f,0x76,0x64,0x37,0x79,0x78,0x63,0x37,0x75,0x72,0x72,0x79,0x37,0x7e,0x7a,0x67,0x7b,0x72,0x7a,0x72,0x79,0x63,0x72,0x73]

private func marginOfSafety(load num: UInt8) -> UInt8 {
    return num ^ 23
}

/*: "Edit Title" :*/
fileprivate let str_poseValue:String = "cell icon mainEdit "

/*: "0/ :*/
fileprivate let str_makeTimeTitle:[Character] = ["0","/"]

/*: "Finish" :*/
fileprivate let str_mData:String = "Finishm bottom make count time"

/*: "btn_delete" :*/
fileprivate let str_tableName:String = "var verticalbtn_del"
fileprivate let str_cornerData:[Character] = ["e","t","e"]

/*: / :*/
fileprivate let str_labName:String = "result"

/*: "\n" :*/
fileprivate let str_messageText:String = "\n"
...
class CustomReactiveCompatible: UIView {
    ...
    override init(frame: CGRect) {
        ...
        super.init(frame: frame)
        //: self.setupSubviews()
        self.timeViewSubviews()
        //: self.setupSubViewsConstraint()
        self.setupConstraint()
        //: let tapGesture = UITapGestureRecognizer(target: self, action: #selector(dismissView))
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(cancelTo))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_titleName.map{marginOfSafety(load: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(str_poseValue.suffix(5)) + "Title").localized
    ...
    lazy var inputTView: EqualLeadingReactiveCompatible = {
        //: let textView = TalkingTextView()
        let textView = EqualLeadingReactiveCompatible()
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(str_mData.prefix(6))).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.nameColors(colors: UIColor.modelColor(), size: CGSize(width: 267, height: 50)), for: .normal)
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.nearCounterval(name: (String(str_tableName.suffix(7)) + String(str_cornerData))), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(bagClose), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension CustomReactiveCompatible: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = me(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func me(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
