...
fileprivate let str_segmentData:[UInt8] = [0xf7,0xf0,0xf7,0xea,0xb6,0xfd,0xf1,0xfa,0xfb,0xec,0xa4,0xb7,0xbe,0xf6,0xff,0xed,0xbe,0xf0,0xf1,0xea,0xbe,0xfc,0xfb,0xfb,0xf0,0xbe,0xf7,0xf3,0xee,0xf2,0xfb,0xf3,0xfb,0xf0,0xea,0xfb,0xfa]

private func kitMake(status num: UInt8) -> UInt8 {
    return num ^ 158
}

/*: "icon_userdetail_header_shadow" :*/
fileprivate let str_equalData:[Character] = ["i","c","o","n","_","u","s","e","r","d","e","t","a","i","l","_","h","e","a","d","e","r","_"]
fileprivate let str_taskData:String = "topadow"

/*: "icon_lounge" :*/
fileprivate let str_popTitle:[Character] = ["i","c","o","n","_","l","o"]
fileprivate let str_cellValue:String = "uaddge"

/*: "Official" :*/
fileprivate let str_managerUseName:[Character] = ["O","f","f","i","c","i","a"]
fileprivate let str_liveValue:String = "path"

/*: "Reviewing" :*/
fileprivate let str_fieldValue:[Character] = ["R"]
fileprivate let str_upPointData:[Character] = ["e","v","i","e","w","i","n","g"]

/*: "#FF506D" :*/
fileprivate let str_modelText:String = "#FF506Ddata near reply color"

/*: "icon_user_detail_intimate_header" :*/
fileprivate let str_indicatorName:[UInt8] = [0x72,0x65,0x64,0x61,0x65,0x68,0x5f,0x65,0x74,0x61,0x6d,0x69,0x74,0x6e,0x69,0x5f,0x6c,0x69,0x61,0x74,0x65,0x64,0x5f,0x72,0x65,0x73,0x75,0x5f,0x6e,0x6f,0x63,0x69]

/*: "?toUid= :*/
fileprivate let str_translationValue:String = "view model center?toUid="

/*: "EnableReactiveCompatible" :*/
fileprivate let str_imageValue:String = "component write behind index viewTUserD"
fileprivate let str_titleValue:String = "lHeauser block view"
fileprivate let str_normalTitle:[Character] = ["d","e","r","I","t"]
fileprivate let str_modeContent:[Character] = ["e","m","C","e","l","l"]

/*: "#FFFFFF" :*/
fileprivate let str_meValue:String = "equal self#FFFFFF"

/*: "#BDB8B5" :*/
fileprivate let str_sizeFacultyTitle:[Character] = ["#","B","D","B","8","B","5"]

/*: "head_detail_bottom" :*/
fileprivate let str_imageTitle:String = "heview"
fileprivate let str_atName:String = "class info todayil_bo"

/*: "#2DF2FF" :*/
fileprivate let str_modeValue:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let str_targetTitle:[Character] = ["L","i","v","e"]

/*: "#48FF1E" :*/
fileprivate let str_sizeValue:[Character] = ["#","4","8","F","F","1","E"]

/*: "Online" :*/
fileprivate let str_mediumValue:String = "video cellOnline"

/*: "#FF4B7F" :*/
fileprivate let str_numberTitle:String = "#"
fileprivate let str_firstValue:String = "atat4B7at"

/*: "busy" :*/
fileprivate let str_fromData:[Character] = ["b","u","s","y"]
...
let kLet_bottomTitle = (302.0 + kLet_messageContent)
...
class UserView: UIView {
    ...
    private var userInfoModel = MModelType()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_segmentData.map{kitMake(status: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        let frame = CGRect(x: 0, y: 0, width: kLet_senseContent, height: kLet_bottomTitle)
    ...
    private lazy var shadowBgView: UIImageView = {
        //: let v = UIImageView(image: UIImage.nearCounterval(name: "icon_userdetail_header_shadow"))
        let v = UIImageView(image: UIImage.nearCounterval(name: (String(str_equalData) + str_taskData.replacingOccurrences(of: "top", with: "sh"))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: kLet_bottomTitle - 15, width: kLet_senseContent, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.nearCounterval(name: (String(str_popTitle) + str_cellValue.replacingOccurrences(of: "add", with: "n")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(str_managerUseName) + str_liveValue.replacingOccurrences(of: "path", with: "l")).localized, for: .normal)
    ...
    private lazy var onlineImgV: RoomThen = {
    ...
    private lazy var liveView: MiniskirtMiniView = {
        //: let v = UserLivePullStreamsView()
        let v = MiniskirtMiniView()
        //: v.isHidden = true
        v.isHidden = true
        //: v.playEndBlock = {
        v.playEndBlock = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: LeadingEqualReactiveCompatible.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            LeadingEqualReactiveCompatible.share.artefactType(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(str_fieldValue) + String(str_upPointData)).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(str_modelText.prefix(7))))
    ...
    lazy var lastLoginLab: UILabel = {
        //: let lb = UILabel()
        let lb = UILabel()
        //: lb.font = .liveAttention(type: .Medium, fontSize: 12)
        lb.font = .liveAttention(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
    ...
    private lazy var intimateBtn: RoomThen = {
        //: let btn = TalkingButton()
        let btn = RoomThen()
        //: btn.setImage(UIImage.nearCounterval(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.nearCounterval(name: String(bytes: str_indicatorName.reversed(), encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
...
extension UserView {
    ...
    private func chopGift() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = ViewManagerReactiveCompatible(videoPath: self.userInfoModel.videoUrl)
        //: if WithAdministratorAppManager.share.loginUid != self.userInfoModel.uid {
        if WithAdministratorAppManager.share.loginUid != self.userInfoModel.uid {
    ...
    private func statusEqualPhoto(_ index: Int) {
        ...
        let vc = CountryViewController(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.actionController()?.navigationController?.pushViewController(vc, animated: true)
    }
    ...
    @objc func effect() {
        ...
        let url = BarbManager.subPurchaseArray(urlType: .Intimate) + (String(str_translationValue.suffix(7))) + "\(self.userInfoModel.uid)"
...
extension UserView: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: EnableReactiveCompatible.self, reuseIdentifier: (String(str_imageValue.suffix(6)) + "etai" + String(str_titleValue.prefix(4)) + String(str_normalTitle) + String(str_modeContent)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        if index == 0 && userInfoModel.sex == MiniSubqueryConvertible.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            ...
            bannerCell.leave(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.commentGesture(urlStr: urlStr, placeImg: UIImage.firstMake())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == WithAdministratorAppManager.share.loginUid else {
        guard userInfoModel.uid == WithAdministratorAppManager.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(str_meValue.suffix(7))))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(str_sizeFacultyTitle)))!
...
extension UserView: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == MiniSubqueryConvertible.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            chopGift()
            ...
            if userInfoModel.sex == MiniSubqueryConvertible.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
            statusEqualPhoto(curren)
        }
    }
...
extension UserView {
    ...
    func paradigmByView(_ userModel: MModelType) {
        ...
        if userModel.sex == MiniSubqueryConvertible.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        bannerView.reloadView()
        // vip
        //: let normalImg = UIImage.nearCounterval(name: "head_detail_bottom").withTintColor(.white)
        let normalImg = UIImage.nearCounterval(name: (str_imageTitle.replacingOccurrences(of: "view", with: "ad") + "_deta" + String(str_atName.suffix(5)) + "ttom")).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = WithAdministratorAppManager.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = WithAdministratorAppManager.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.commentGesture(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .insert() : .white
...
//        }
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.apply(color: UIColor(hex: (String(str_modeValue)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.smart(), for: .normal)
            onlineImgV.setImage(image.smart(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(str_targetTitle)).localized, for: .normal)
            ...
            let image = UIImage.apply(color: UIColor(hex: (String(str_sizeValue)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.smart(), for: .normal)
            onlineImgV.setImage(image.smart(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(str_mediumValue.suffix(6))).localized, for: .normal)
            ...
            let image = UIImage.apply(color: UIColor(hex: (str_numberTitle.capitalized + str_firstValue.replacingOccurrences(of: "at", with: "F")))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.smart(), for: .normal)
            onlineImgV.setImage(image.smart(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((String(str_fromData)).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if WithAdministratorAppManager.share.loginUid != userModel.uid &&
        if WithAdministratorAppManager.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.physicalStructureUrl(liveUrl: userModel.livePlayUrl)
        }
    }
    ...
    private func coordinate() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(kLet_messageContent + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(kLet_messageContent + 15)
