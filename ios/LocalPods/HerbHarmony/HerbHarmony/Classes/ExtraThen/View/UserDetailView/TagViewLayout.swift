...
protocol TextLayoutDelegate: NSObjectProtocol {
    //: func getCollectuonViewHeight(layoutH: CGFloat)
    func engageWithinLayoutPosition(layoutH: CGFloat)
}
...
class TagViewLayout: UICollectionViewFlowLayout {
    ...
    var delegate: TextLayoutDelegate?
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for i in 1..<attributes.count {
            ...
            if i == attributes.count-1 {
                ...
                self.delegate?.engageWithinLayoutPosition(layoutH: curAttr.frame.maxY)
            }
        }
