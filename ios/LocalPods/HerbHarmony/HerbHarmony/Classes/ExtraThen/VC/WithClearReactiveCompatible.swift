...
fileprivate let str_intimateValue:[Character] = ["i","c","o","n","_","k","o","n","g","_","k","o","n","g","_","d","e","f","a","u","l","t"]

/*: "page" :*/
fileprivate let str_attentionData:[UInt8] = [0xff,0xee,0xe8,0xea]

private func backgroundSucceed(segment num: UInt8) -> UInt8 {
    return num ^ 143
}

/*: "uid" :*/
fileprivate let str_progressData:[UInt8] = [0x9,0xfd,0xf8]

fileprivate func pauseStyleOf(message num: UInt8) -> UInt8 {
...
/*: "list" :*/
fileprivate let str_locationTitle:String = "LIST"
...
// __CLOSE_PRINT__
//
//  WithClearReactiveCompatible.swift
//  AbroadTalking
//
...
class WithClearReactiveCompatible: NameViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [PopSignTransformable]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: kLet_clickSoundViewText, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(IconView.self, forCellWithReuseIdentifier: IconView.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.nearCounterval(name: "icon_kong_kong_default")
        img.image = UIImage.nearCounterval(name: (String(str_intimateValue)))
...
extension WithClearReactiveCompatible {
    ...
    private func embedData() {
        //: let params = ["page": pageIndex, "uid": userInfoModel.uid] as [String: Any]
        let params = [String(bytes: str_attentionData.map{backgroundSucceed(segment: $0)}, encoding: .utf8)!: pageIndex, String(bytes: str_progressData.map{pauseStyleOf(message: $0)}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        ManagerRequestTool.mortal(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(str_locationTitle.lowercased())] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = ModelHandyJSON.deserialize(from: dict as? Dictionary) {
...
extension WithClearReactiveCompatible: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: IconView.className(), for: indexPath) as! IconView
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = ShowIconFactViewDelegate(model: detailModel)
...
extension WithClearReactiveCompatible: JXPagingViewListViewDelegate {
...
extension WithClearReactiveCompatible {
    ...
    func alongController(_ userModel: MModelType) {
    ...
    private func pointOfNumberersection() {
