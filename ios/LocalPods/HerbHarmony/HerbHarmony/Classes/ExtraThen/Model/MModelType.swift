...
/*: "id" :*/
fileprivate let str_timingName:[Character] = ["i","d"]
...
struct MModelType: HandyJSON {
    ...
    var momentPic: MessageMeasurable?
    //: var gallery: Array<TalkingUserInfoGalleryModel>?
    var gallery: [PossibilityHandyJSON]? // 相册
    ...
    var aiExtraInfo = IndexHandyJSON() // AI模型
    //: var vipSkinId = 0
...
struct MessageMeasurable: HandyJSON {
    //: var count: Int = 0
    var count: Int = 0
    //: var list: Array<TalkingUserInfoMomentItemModel>?
    var list: [PopSignTransformable]?
}

// MARK: - 动态Item

//: struct TalkingUserInfoMomentItemModel: HandyJSON {
struct PopSignTransformable: HandyJSON {
...
struct PossibilityHandyJSON: HandyJSON {
    ...
    func laboratoryBench(compareTo: PossibilityHandyJSON) -> Bool {
        //: return
        return
...
enum MoviegoerSubmitSlotNameConvertible: Int {
    //: case Normal    = 0
    case Normal = 0 // 正常
    //: case Reviewing = 1
