...
fileprivate let str_iconData:[UInt8] = [0x4e,0x49,0x4e,0x53,0xf,0x44,0x48,0x43,0x42,0x55,0x1d,0xe,0x7,0x4f,0x46,0x54,0x7,0x49,0x48,0x53,0x7,0x45,0x42,0x42,0x49,0x7,0x4e,0x4a,0x57,0x4b,0x42,0x4a,0x42,0x49,0x53,0x42,0x43]

private func guardFrameMake(reply num: UInt8) -> UInt8 {
    return num ^ 39
}

/*: "gift_bg_dm" :*/
fileprivate let str_sharedData:[Character] = ["g","i","f","t","_","b","g"]
fileprivate let str_resultText:String = "first finish let view pic_dm"

/*: "#FFF4B5" :*/
fileprivate let str_viewValue:[Character] = ["#","F"]
fileprivate let str_rawName:[Character] = ["F","F","4","B","5"]

/*: "won " :*/
fileprivate let str_playerName:String = "won var user"

/*: " coins in " :*/
fileprivate let str_endText:String = "file cell design equal coi"

/*: " x :*/
fileprivate let str_viewStatusName:[Character] = [" ","x"]

/*: "just sent" :*/
fileprivate let str_postData:String = "JUST"

/*: "  :*/
fileprivate let str_shareSharedData:[Character] = [" "]

/*: "#4120C1" :*/
fileprivate let str_modelTitle:String = "#4data20"
fileprivate let str_titleValue:String = "path"
...
class AlongReactiveCompatible: UIView {
    ...
    private var giftModel = SocietalBroadcastModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_iconData.map{guardFrameMake(reply: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        //: let v = UIImageView()
        let v = UIImageView()
        //: v.image = UIImage.nearCounterval(name: "gift_bg_dm")
        v.image = UIImage.nearCounterval(name: (String(str_sharedData) + String(str_resultText.suffix(3))))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(str_viewValue) + String(str_rawName)))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(str_viewValue) + String(str_rawName)))
...
extension AlongReactiveCompatible {
    ...
    func nameGift(_ model: SocietalBroadcastModel) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.nearSquare(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(str_playerName.prefix(4))).localized + "\(giftModel.totalCoin)" + (String(str_endText.suffix(4)) + "ns in ").localized + giftModel.gameName
            ...
            giftImgV.nearSquare(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (str_postData.lowercased() + " sent").localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = assemblagePath(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (str_modelTitle.replacingOccurrences(of: "data", with: "1") + str_titleValue.replacingOccurrences(of: "path", with: "C1")))!, range: nsRange)
            }
    ...
    private func assemblagePath(giftmodel: SocietalBroadcastModel) -> String {
...
extension AlongReactiveCompatible {
    ...
    private func aLaCarte() {
        //: self.frame = CGRect(x: 0, y: kLet_likeTitle, width: ScreenWidth, height: 60)
        self.frame = CGRect(x: 0, y: kLet_likeTitle, width: kLet_senseContent, height: 60)
    ...
    private func buildSubConstraint() {
