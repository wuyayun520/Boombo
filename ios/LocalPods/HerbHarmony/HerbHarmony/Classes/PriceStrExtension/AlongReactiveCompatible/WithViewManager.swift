...
/*: "position.x" :*/
fileprivate let str_pointValue:String = "pwhitesitiwhiten"
fileprivate let str_tabData:[Character] = [".","x"]

/*: "slideIn" :*/
fileprivate let str_messageText:String = "slideInlive open"

/*: "slideOut" :*/
fileprivate let str_liveData:String = "sjumpd"
...
class WithViewManager: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = WithViewManager()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [SocietalBroadcastModel]()
    ...
    private lazy var giftView: AlongReactiveCompatible = {
        //: let v = GiftFloatView()
        let v = AlongReactiveCompatible()
        //: IconThen.getWindow().addSubview(v)
        IconThen.getBind().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension WithViewManager {
    ...
    private func appSince(_ model: SocietalBroadcastModel) {
        ...
        let isRTL = MeanSunLanguageManager.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : kLet_senseContent + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = kLet_senseContent / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? kLet_senseContent + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (str_pointValue.replacingOccurrences(of: "white", with: "o") + String(str_tabData)))
        ...
        self.giftView.layer.add(slideIn, forKey: (String(str_messageText.prefix(7))))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (str_pointValue.replacingOccurrences(of: "white", with: "o") + String(str_tabData)))
            ...
            self.giftView.layer.add(slideOut, forKey: (str_liveData.replacingOccurrences(of: "jump", with: "li") + "eOut"))
...
extension WithViewManager {
    ...
    func combineNotifications() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(self.recievedNewGift(noti:)),
                                               selector: #selector(self.exceptView(noti:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.areaState),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.momentTarget),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.examine),
    ...
    @objc private func exceptView(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<SocietalBroadcastModel>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != TotalStrideable.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = WithAdministratorAppManager.share.appUserConfigMode.userType
        let currUserType = WithAdministratorAppManager.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != WithAdministratorAppManager.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != WithAdministratorAppManager.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
        ...
        self.imageAcross(gift: gift)
    }

    /// 应用退到后台
    //: @objc private func enterBackground() {
    @objc private func areaState() {
    ...
    @objc private func momentTarget() {
        //: self.activeWhenShow()
        self.textAwake()
    }
