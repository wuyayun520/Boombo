...
fileprivate let str_containerContent:[UInt8] = [0x22,0x27,0x22,0x2d,0xe1,0x1c,0x28,0x1d,0x1e,0x2b,0xf3,0xe2,0xd9,0x21,0x1a,0x2c,0xd9,0x27,0x28,0x2d,0xd9,0x1b,0x1e,0x1e,0x27,0xd9,0x22,0x26,0x29,0x25,0x1e,0x26,0x1e,0x27,0x2d,0x1e,0x1d]

fileprivate func northNorthwest(equal num: UInt8) -> UInt8 {
...
protocol PersonCellDelegete: NSObjectProtocol {
    ///
...
class SignView: UIView {
    ...
    var maxRemoveDistance = kLet_senseContent / 4.0
    //: var cell_delegate: TalkingClubCardViewCellDelegete?
    var cell_delegate: PersonCellDelegete?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_containerContent.map{northNorthwest(equal: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(omeletPan(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension SignView: UIGestureRecognizerDelegate {
...
//        let sc = self.model?.isScroll
    ...
    func addDisputeFurculaOcularMuscleWeltanschauung() {
    ...
    @objc func omeletPan(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
            ...
            self.cell_delegate?.errorSizeAction(cell: self)
        //: case .changed:
        case .changed:
...
//            } else {
            ...
            if self.currentPoint.x < -50 {
                ...
                self.cell_delegate?.premiumAt(cell: self, forMovePoint: self.currentPoint, direction: .Left)
                ...
                self.cell_delegate?.premiumAt(cell: self, forMovePoint: self.currentPoint, direction: .Right)
            }
            //: pan.setTranslation(.zero, in: pan.view)
            pan.setTranslation(.zero, in: pan.view)
//            }
...
//            }
            ...
            self.cell_delegate?.way(cell: self)
    ...
    func saucepanAt() {
        ...
        if self.currentPoint.x < -self.maxRemoveDistance {
            ...
            conversationChange(direction: .Left, isGesture: true)
            ...
            conversationChange(direction: .Right, isGesture: true)
    ...
    func conversationChange(direction: MediaConstantTarget, isGesture: Bool) {
    ...
    func outspoken(direction: MediaConstantTarget) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
            //: removeFromSuperviewLeft(direction: .Left)
            directDirection(direction: .Left)
        //: case .Right:
        case .Right:
            //: removeFromSuperviewRight(direction: .Right)
            itemRefuseInfo(direction: .Right)
        }
    }

    // 向左边移除动画
    //: func removeFromSuperviewLeft(direction: CardCellSwipeDirection) {
    func directDirection(direction: MediaConstantTarget) {
        //: let snapshotView = self.snapshotView(afterScreenUpdates: false)
        let snapshotView = self.snapshotView(afterScreenUpdates: false)
        //: if snapshotView != nil {
        if snapshotView != nil {
        ...
        conversationChange(direction: direction, isGesture: false)
    ...
    func itemRefuseInfo(direction: MediaConstantTarget) {
        //: let snapshotView = self.snapshotView(afterScreenUpdates: false)
        let snapshotView = self.snapshotView(afterScreenUpdates: false)
