...
fileprivate let str_edgeTitle:[UInt8] = [0xe,0x13,0xe,0x19,0xcd,0x8,0x14,0x9,0xa,0x17,0xdf,0xce,0xc5,0xd,0x6,0x18,0xc5,0x13,0x14,0x19,0xc5,0x7,0xa,0xa,0x13,0xc5,0xe,0x12,0x15,0x11,0xa,0x12,0xa,0x13,0x19,0xa,0x9]

fileprivate func makeViewAccept(lab num: UInt8) -> UInt8 {
...
/*: "img_home_shadow" :*/
fileprivate let str_pushData:String = "img_hwindow equal sex"
fileprivate let str_upValue:String = "shadbar"

/*: "icon_match_dislikeBtn_nor" :*/
fileprivate let str_bringCancelValue:[Character] = ["i","c","o","n","_","m","a","t","c","h","_","d","i","s","l"]
fileprivate let str_equalText:String = "pick"
fileprivate let str_pageValue:[Character] = ["e","B","t","n","_","n","o","r"]

/*: "icon_match_likeBtn_nor" :*/
fileprivate let str_mediumName:String = "icon_mamodel case let height"
fileprivate let str_styleServiceLetName:[Character] = ["t","c","h","_","l","i","k","e","B","t","n","_","n","o","r"]
...
// __CLOSE_PRINT__
//
//  PreservationReactiveCompatible.swift
//  AbroadTalking
//
...
protocol ManagerVolumeDataSource: NSObjectProtocol {
    ///
    //: func numberOfCountInCardView(cardView: TalkingClubCardView) -> Int
    func numberVideoView(cardView: PreservationReactiveCompatible) -> Int
...
@objc protocol FromCollectionViewDelegate: NSObjectProtocol {
    ///
    ...
    @objc optional func titleTarget(cardView: PreservationReactiveCompatible, cardViewCell: SignView, forRowAtIndex: Int, direction: Int, isGesture: Bool)

    //: @objc optional func didRemoveLastCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func outWrite(cardViewCell: SignView, forRowAtIndex: Int)

    //: @objc optional func didDisplayCell(cardViewCell: TalkingClubCardViewCell, forRowAtIndex: Int)
    @objc optional func finishBlock(cardViewCell: SignView, forRowAtIndex: Int)

    //: @objc optional func didMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell, forMovePoint: CGPoint)
    @objc optional func movePoint(cardView: PreservationReactiveCompatible, cardViewCell: SignView, forMovePoint: CGPoint)

    //: @objc optional func beginMoveCell(cardView: TalkingClubCardView, cardViewCell: TalkingClubCardViewCell)
    @objc optional func climax(cardView: PreservationReactiveCompatible, cardViewCell: SignView)
}

//: class TalkingClubCardView: UIView {
class PreservationReactiveCompatible: UIView {
    ...
    var maxRemoveDistance = kLet_senseContent / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [SignView]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: ManagerVolumeDataSource? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: FromCollectionViewDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_edgeTitle.map{makeViewAccept(lab: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.contentMode = .scaleAspectFill
        //: imag.image = UIImage.nearCounterval(name: "img_home_shadow")
        imag.image = UIImage.nearCounterval(name: (String(str_pushData.prefix(5)) + "ome_" + str_upValue.replacingOccurrences(of: "bar", with: "ow")))
    ...
    private lazy var disLikeImage: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.nearCounterval(name: "icon_match_dislikeBtn_nor")
        img.image = UIImage.nearCounterval(name: (String(str_bringCancelValue) + str_equalText.replacingOccurrences(of: "pic", with: "i") + String(str_pageValue)))
        //: img.isHidden = true
        img.isHidden = true
        //: img.alpha = 0.5
        img.alpha = 0.5
    ...
    private lazy var likeImage: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.nearCounterval(name: "icon_match_likeBtn_nor")
        img.image = UIImage.nearCounterval(name: (String(str_mediumName.prefix(7)) + String(str_styleServiceLetName)))
        //: img.isHidden = true
        img.isHidden = true
        //: img.alpha = 0.5
        img.alpha = 0.5
...
extension PreservationReactiveCompatible {
    //: func reloadDataAnimated(anim: Bool) {
    func pauseTo(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.numberVideoView(cardView: self) ?? 0
    ...
    func playIndex(index: Int) {
        ...
        let cell = dataSource?.fromWith(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? SignView(), at: 0)
    ...
    func userAnim(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: SignView = self.visibleCells[i] as! SignView
            ...
            if isVisbleLast {
                ...
                self.delegate?.finishBlock?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                nestled(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func nestled(cell: SignView, transform: CGAffineTransform) {
    ...
    func mismanage(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func emptyDoing(identifier: String) -> SignView? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! DirectorItemCell.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: DirectorItemCell = cellclass.init(reuseIdentifier: identifier)
...
extension PreservationReactiveCompatible: PersonCellDelegete {
    //: func cardViewCellDidRemoveFromSuperView(cell: TalkingClubCardViewCell, direction: CardCellSwipeDirection, isGesture: Bool) {
    func upGesture(cell: SignView, direction: MediaConstantTarget, isGesture: Bool) {
        ...
        self.delegate?.titleTarget?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1, direction: direction.rawValue, isGesture: isGesture)
        ...
        if self.visibleCells.count == 0 { // 数据看完了
            ...
            self.delegate?.outWrite?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.numberVideoView(cardView: self) ?? 0

        // 当前数据源还有数据 继续创建cell
        //: if (self.currentIndex < count - 1) {
        if self.currentIndex < count - 1 {
            //: createCardViewCell(index: self.currentIndex + 1)
            playIndex(index: self.currentIndex + 1)
        }
        // 更新布局
    ...
    func premiumAt(cell: SignView, forMovePoint: CGPoint, direction: MediaConstantTarget) {
        //: self.delegate?.didMoveCell?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        self.delegate?.movePoint?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        //: if  direction == .Left{
        if direction == .Left {
    ...
    func errorSizeAction(cell: SignView) {
        //: self.delegate?.beginMoveCell?(cardView: self, cardViewCell: cell)
        self.delegate?.climax?(cardView: self, cardViewCell: cell)
    }
}
