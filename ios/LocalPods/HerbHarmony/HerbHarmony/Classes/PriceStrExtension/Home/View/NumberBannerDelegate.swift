...
fileprivate let str_dropText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "JXBannerCellID" :*/
fileprivate let str_viewData:String = "var target bottom model indexJXBa"
fileprivate let str_centerToolTitle:String = "ellIDto equal cell"

/*: "#FFFFFF" :*/
fileprivate let str_toName:[Character] = ["#","F","F","F","F","F","F"]

/*: "#BDB8B5" :*/
fileprivate let str_availableText:String = "image var#BDB8B5"
...
protocol QuantityryObjectProtocol: NSObjectProtocol {
    // 加载更多数据
...
class NumberBannerDelegate: UICollectionReusableView {
    //: weak var delegate: SocialPopularListFooterViewProtocol?
    weak var delegate: QuantityryObjectProtocol?
    //: var listData: [PremiumStarListModel]?
    var listData: [SuperfluousModelType]? // 巨星计划数据
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_dropText.reversed(), encoding: .utf8)!)
    }
...
extension NumberBannerDelegate {
    /// 获取当前视图高度
    /// - Returns: 高度
    //: func getFooterViewHeight() -> CGFloat {
    func mathematicHeight() -> CGFloat {
    ...
    func labWith(data: [SuperfluousModelType]) {
        //: self.listData = data
        self.listData = data
    }
    ...
    func statePoint(data: [CoverModelType]) {
...
extension NumberBannerDelegate: JXBannerDataSource, JXBannerDelegate {
    //: func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        //: return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: "JXBannerCellID")
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(str_viewData.suffix(4)) + "nnerC" + String(str_centerToolTitle.prefix(5))))
    }
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(str_toName)))!
        //: pageControl.inactiveSize = CGSize(width: 6, height: 6)
        pageControl.inactiveSize = CGSize(width: 6, height: 6)
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(str_availableText.suffix(7))))!
                         ...
                         didSelectItemAt index: Int)
    {
        ...
        delegate?.equalTime(model: model)
    }
}
...
extension NumberBannerDelegate {
    ...
    private func userPush() {
        ...
        bannerView.snp.makeConstraints { make in
            ...
            make.size.equalTo(CGSize(width: kLet_senseContent - 30, height: 84))
        }
    }
}
