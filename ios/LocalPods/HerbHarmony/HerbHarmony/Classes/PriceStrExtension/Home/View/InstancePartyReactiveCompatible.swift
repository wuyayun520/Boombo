...
fileprivate let str_limitData:[UInt8] = [0x61,0x66,0x61,0x7c,0x20,0x6b,0x67,0x6c,0x6d,0x7a,0x32,0x21,0x28,0x60,0x69,0x7b,0x28,0x66,0x67,0x7c,0x28,0x6a,0x6d,0x6d,0x66,0x28,0x61,0x65,0x78,0x64,0x6d,0x65,0x6d,0x66,0x7c,0x6d,0x6c]

/*: "Welcome to my room" :*/
fileprivate let str_keyValue:[Character] = ["W","e","l","c","o","m","e"," ","t","o"]
fileprivate let str_clearValue:String = " my block image"

/*: "%@'s room" :*/
fileprivate let str_indexValue:String = "%@\'s"

/*: "CCCCCC" :*/
fileprivate let str_pathText:[Character] = ["C","C","C","C","C","C"]

/*: "#E1D0FC" :*/
fileprivate let str_arrayData:String = "cover method add server#E1D0"
fileprivate let str_formatValue:String = "lab"

/*: "#FFE3E5" :*/
fileprivate let str_effectKeyName:[Character] = ["#","F","F","E","3"]
fileprivate let str_exitContent:[Character] = ["E","5"]

/*: "get json error" :*/
fileprivate let str_viewAttentionText:String = "get jslanguage self"
fileprivate let str_removeData:[Character] = ["r","o","r"]
...
// __CLOSE_PRINT__
//
//  InstancePartyReactiveCompatible.swift
//  HerbHarmony
//
...
class InstancePartyReactiveCompatible: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        wineStewardConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: str_limitData.map{$0^8}, encoding: .utf8)!)
    }
    ...
    var listModel: ShowMeasurable? {
        //: didSet {
        didSet {
            //: guard let listModel else { return }
            guard let listModel else { return }

            //: if String(listModel.uid) == WithAdministratorAppManager.share.loginUid {
            if String(listModel.uid) == WithAdministratorAppManager.share.loginUid {
                ...
                countryLab.text = (String(str_keyValue) + String(str_clearValue.prefix(4)) + "room").localized
                //: countryLab.textColor = .appValueColor()
                countryLab.textColor = .colorPush()
                //: countryLab.font = UIFont.liveAttention(type: .Medium, fontSize: 15)
                countryLab.font = UIFont.liveAttention(type: .Medium, fontSize: 15)
                ...
                countryLab.text = "\(listModel.country)"
                //: countryLab.textColor = .inexpensiveness()
                countryLab.textColor = .inexpensiveness()
                //: countryLab.font = UIFont.liveAttention(type: .Regular, fontSize: 15)
                countryLab.font = UIFont.liveAttention(type: .Regular, fontSize: 15)
                //: countryLab.snp.updateConstraints { make in
                countryLab.snp.updateConstraints { make in
            ...
            headPicImgV.nearSquare(urlStr: listModel.pic)
            //: nameLab.text = "%@'s room".localizedArguments(listModel.name)
            nameLab.text = (str_indexValue + " room").localizedArguments(listModel.name)
            //: countryImgView.setUrlImage(urlStr: listModel.flag)
            countryImgView.nearSquare(urlStr: listModel.flag)
            //: hotLab.text = "\(listModel.score)"
            hotLab.text = "\(listModel.score)"

            //: let hotWidth  = hotLab.sizeThatFits(CGSize(width: ScreenWidth, height: 20)).width
            let hotWidth = hotLab.sizeThatFits(CGSize(width: kLet_senseContent, height: 20)).width
    ...
    private lazy var bgView: UIView = {
        ...
        v.layer.shadowColor = UIColor(hex: (String(str_pathText)))?.withAlphaComponent(0.5).cgColor
    ...
    private lazy var bgvlayer: CAGradientLayer = {
        ...
        layer.frame = CGRect(x: 0, y: 0, width: kLet_senseContent - 24, height: 76)
        //: layer.startPoint = CGPoint(x: 0, y: 0)
        layer.startPoint = CGPoint(x: 0, y: 0)
        //: layer.endPoint = CGPoint(x: 1, y: 0.5)
        layer.endPoint = CGPoint(x: 1, y: 0.5)
        //: layer.colors = [UIColor(hex: "#E1D0FC")?.cgColor, UIColor(hex: "#FFE3E5")?.cgColor]
        layer.colors = [UIColor(hex: (String(str_arrayData.suffix(5)) + str_formatValue.replacingOccurrences(of: "lab", with: "FC")))?.cgColor, UIColor(hex: (String(str_effectKeyName) + String(str_exitContent)))?.cgColor]
    ...
    private lazy var liveSvga: SVGAPlayer = {
        ...
        do {
            ...
            let url = DigitiserReactiveCompatible.default.game(type: .Party_List)
            ...
            printLog(message: (String(str_viewAttentionText.prefix(6)) + "on er" + String(str_removeData)))
        }
    ...
    private lazy var hotLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.font = UIFont.liveAttention(type: .Medium, fontSize: 13)
        lab.font = UIFont.liveAttention(type: .Medium, fontSize: 13)
...
extension InstancePartyReactiveCompatible {
    // 添加视图
    ...
    private func wineStewardConstraint() {
