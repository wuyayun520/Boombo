
//: Declare String Begin

/*: "New friends" :*/
fileprivate let str_mWithText:String = "wrap imageNew frien"
fileprivate let str_colorValue:String = "main"

/*: "icon_yidu_pre" :*/
fileprivate let str_textName:String = "ICON"
fileprivate let str_modelValue:String = "u_prcase new view send"
fileprivate let str_jumpName:[Character] = ["e"]

/*: "You've got no message yet." :*/
fileprivate let str_makeText:[Character] = ["Y","o","u","\'","v","e"," ","g","o","t"," "]
fileprivate let str_errorText:String = "no mesin send"

/*: "icon_kong_kong_default" :*/
fileprivate let str_observerText:String = "ivideon"
fileprivate let str_jumpTapName:String = "g_koninfo hidden"
fileprivate let str_equalName:[Character] = ["l","t"]

/*: "Cancel" :*/
fileprivate let str_viewData:String = "make toCancel"

/*: "OK" :*/
fileprivate let str_receiveBorderErrorValue:String = "Ocell"

/*: "uid" :*/
fileprivate let str_sizeSearchValue:[UInt8] = [0x41,0x5d,0x50]

private func backRoom(title num: UInt8) -> UInt8 {
    return num ^ 52
}

/*: "Do you want to mark all messages as read?" :*/
fileprivate let str_frameMakeValue:[UInt8] = [0x33,0x18,0x57,0xe,0x18,0x2,0x57,0x0,0x16,0x19,0x3,0x57,0x3,0x18,0x57,0x1a,0x16,0x5,0x1c,0x57,0x16,0x1b,0x1b,0x57,0x1a,0x12,0x4,0x4,0x16,0x10,0x12,0x4,0x57,0x16,0x4,0x57,0x5,0x12,0x16,0x13,0x48]

private func broadcastIndex(constitutional num: UInt8) -> UInt8 {
    return num ^ 119
}

/*: "MoreMessages一键已读失败：code: :*/
fileprivate let str_sendTitle:[Character] = ["M","o","r","e","M","e","s","s","a","g","e","s","一","\u{952e}","已","读","失"]
fileprivate let str_appearImageName:String = "black play right of败：code:"

/*: , desc: :*/
fileprivate let str_tabText:[Character] = [","," ","d","e","s","c"]
fileprivate let str_frameData:[Character] = [":"]

/*: "btn_message_report_nor" :*/
fileprivate let str_nameData:[Character] = ["b","t","n","_","m","e","s","s","a","g","e","_","r","e","p","o"]
fileprivate let str_kitTitleText:String = "rt_norallow coin your let var"

/*: "#FF935D" :*/
fileprivate let str_dropData:[Character] = ["#","F","F","9","3","5","D"]

/*: "btn_message_block_nor" :*/
fileprivate let str_roundData:[Character] = ["b","t","n","_","m","e","s","s","a","g","e","_","b","l","o","c","k","_"]
fileprivate let str_addEmptyName:String = "nstatusr"

/*: "#C179F9" :*/
fileprivate let str_showData:String = "return path#C179F9"

/*: "btn_message_delete_nor" :*/
fileprivate let str_localValue:String = "btn_metable else input"
fileprivate let str_toName:String = "dsharedete"
fileprivate let str_shareData:[Character] = ["_","n","o","r"]

/*: "#FF506D" :*/
fileprivate let str_labelTitle:String = "equal"
fileprivate let str_mValue:String = "FF50selected"

//: Declare String End

// __DEBUG__
// __CLOSE_PRINT__
//
//  TotalUserDataSource.swift
//  AbroadTalking
//
//  Created by young on 2023/9/27.
//

//: import UIKit
import UIKit

//: class TalkingMoreMessagesListViewController: TalkingBaseViewController {
class TotalUserDataSource: NameViewController {
    //: override func viewDidLoad() {
    override func viewDidLoad() {
        //: super.viewDidLoad()
        super.viewDidLoad()
        //: self.title = "New friends".localized
        self.title = (String(str_mWithText.suffix(9)) + str_colorValue.replacingOccurrences(of: "main", with: "ds")).localized
        //: let btn = UIButton(frame: CGRect(x: 0, y: 0, width: 24, height: 24))
        let btn = UIButton(frame: CGRect(x: 0, y: 0, width: 24, height: 24))
        //: btn.setImage(UIImage.nearCounterval(name: "icon_yidu_pre"), for: .normal)
        btn.setImage(UIImage.nearCounterval(name: (str_textName.lowercased() + "_yid" + String(str_modelValue.prefix(4)) + String(str_jumpName))), for: .normal)
        //: btn.addTarget(self, action: #selector(clearBadgeButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(clearCutFromClick), for: .touchUpInside)
        //: let item = UIBarButtonItem(customView: btn)
        let item = UIBarButtonItem(customView: btn)
        //: self.navigationItem.rightBarButtonItem = item
        self.navigationItem.rightBarButtonItem = item

        //: EdificeV2Listener.shared.func__addDelegate(self)
        EdificeV2Listener.shared.frameDelegate(self)
        //: func__installNotificationObservers()
        deed()
        //: createUI()
        stopUser()
        //: self.manager.req_moreMsgInitData()
        self.manager.totalerval()
        //: refreshTableView()
        willView()
    }

    //: deinit {
    deinit {
        //: NotificationCenter.default.removeObserver(self)
        NotificationCenter.default.removeObserver(self)
    }

    // MARK: - Lazy load

    //: private lazy var tableView: UITableView = {
    private lazy var tableView: UITableView = {
        //: let tableView = UITableView.init(frame: .zero, style: .grouped)
        let tableView = UITableView(frame: .zero, style: .grouped)
        //: tableView.delegate = self
        tableView.delegate = self
        //: tableView.dataSource = self
        tableView.dataSource = self
        //: tableView.backgroundColor = .clear
        tableView.backgroundColor = .clear
        //: tableView.separatorStyle = .none
        tableView.separatorStyle = .none
        //: tableView.register(TalkingChatListTableCell.self, forCellReuseIdentifier: TalkingChatListTableCell.className())
        tableView.register(StandardMainViewCell.self, forCellReuseIdentifier: StandardMainViewCell.className())
        //: return tableView
        return tableView
        //: }()
    }()

    //: private lazy var emptyView: EmptyView = {
    private lazy var emptyView: EmptyView = {
        //: var style = EmptyStyle()
        var style = StackEmptyStyle()
        //: style.TipsTitle = "You've got no message yet.".localized
        style.TipsTitle = (String(str_makeText) + String(str_errorText.prefix(6)) + "sage yet.").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (str_observerText.replacingOccurrences(of: "video", with: "co") + "_kon" + String(str_jumpTapName.prefix(5)) + "g_defau" + String(str_equalName))
        //: let emptyView = EmptyView.init(frame: .zero, style: style)
        let emptyView = EmptyView(frame: .zero, style: style)
        //: emptyView.isHidden = true
        emptyView.isHidden = true
        //: return emptyView
        return emptyView
        //: }()
    }()

    //: private lazy var manager: TalkingChatListManager = {
    private lazy var manager: CountRelationListManager = //: return CountRelationListManager()
        .init()
    //: }()
}

// MARK: - 刷新

//: extension TalkingMoreMessagesListViewController {
extension TotalUserDataSource {
    /// 刷新表格
    //: func refreshTableView() {
    func willView() {
        //: guard self.manager.moreMsgArr.count != 0 else {
        guard self.manager.moreMsgArr.count != 0 else {
            //: self.tableView.isHidden = true
            self.tableView.isHidden = true
            //: self.emptyView.isHidden = false
            self.emptyView.isHidden = false
            //: self.reloadData()
            self.informationPerform()
            //: return
            return
        }

        //: self.tableView.isHidden = false
        self.tableView.isHidden = false
        //: self.emptyView.isHidden = true
        self.emptyView.isHidden = true
        //: self.reloadData()
        self.informationPerform()
    }

    /// 刷新
    //: func reloadData() {
    func informationPerform() {
        //: self.tableView.reloadData()
        self.tableView.reloadData()
    }
}

// MARK: - Event

//: extension TalkingMoreMessagesListViewController {
extension TotalUserDataSource {
    // 删除会话cell
    //: func deleteChatListCell(conversationModel: TalkingConversationModel) {
    func circleList(conversationModel: MediaConversationModel) {
        //: if !EdificeV2Listener.shared.func__checkCanOperateList() { return }
        if !EdificeV2Listener.shared.operate() { return }

        //: AbTalkingPrivateChatAnimatTool.shared.removeConversionAnimat(targetID: conversationModel.targetId)
        CompartmentAnimatTool.shared.statusImage(targetID: conversationModel.targetId)
        //: EdificeV2Listener.shared.func__removeConversation(targetId: conversationModel.targetId, dataType: .moreList)
        EdificeV2Listener.shared.tableTalk(targetId: conversationModel.targetId, dataType: .moreList)
        //: self.manager.req_removeConversationModel(conversationModel: conversationModel)
        self.manager.lobby(conversationModel: conversationModel)
        //: self.tableView.reloadData()
        self.tableView.reloadData()
        //: DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.25) {
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.25) {
            // 刷新表格，更新红点
            //: self.refreshTableView()
            self.willView()
        }
    }

    // 拉黑会话
    //: func blockChatListCell(conversationModel: TalkingConversationModel) {
    func swageBlockNextPossibility(conversationModel: MediaConversationModel) {
        // 拉黑/解除拉黑
        //: TalkingAlertShow.alert(title: nil,
        VoiceAlertShow.enablelineMedium(title: nil,
                                          //: message: kMessage_blocking,
                                          message: kLet_pathText,
                                          //: leftBtnTitle: "Cancel".localized,
                                          leftBtnTitle: (String(str_viewData.suffix(6))).localized,
                                          //: rightBtnTitle: "OK".localized) {
                                          rightBtnTitle: "OK".localized)
        {
            //: TalkingAlertShow.hideAlert()
            VoiceAlertShow.bias()
            //: } rightBlock: {
        } rightBlock: {
            //: TalkingAlertShow.hideAlert()
            VoiceAlertShow.bias()

            //: TalkingUserRequestManager.func__pullBlackWithUserId(uid: conversationModel.userID, isBlack: true) { succeed, result, errorModel in
            UnambiguousThen.custom(uid: conversationModel.userID, isBlack: true) { succeed, _, _ in
                //: guard succeed == true else {
                guard succeed == true else {
                    //: return
                    return
                }
                // 拉黑成功，从列表中移除
                //: self.deleteChatListCell(conversationModel: conversationModel)
                self.circleList(conversationModel: conversationModel)
                //: NotificationCenter.default.post(name: LIVE_BLOCK_USER_NOTIFICATION,
                NotificationCenter.default.post(name: kLet_barContent,
                                                //: object: nil,
                                                object: nil,
                                                //: userInfo: ["uid": conversationModel.userID])
                                                userInfo: [String(bytes: str_sizeSearchValue.map{backRoom(title: $0)}, encoding: .utf8)!: conversationModel.userID])
            }
        }
    }

    // 举报会话
    //: func reportChatListCell(conversationModel: TalkingConversationModel) {
    func chat(conversationModel: MediaConversationModel) {
        //: let reportView = TalkingReportAlertView.init(frame: UIScreen.main.bounds, type: .reportUserType, rId: conversationModel.targetId)
        let reportView = SectionThen(frame: UIScreen.main.bounds, type: .reportUserType, rId: conversationModel.targetId)
        //: reportView.showReportViewIn(view: nil)
        reportView.format(view: nil)
    }

    /// 清除红点角标
    //: @objc private func clearBadgeButtonClick() {
    @objc private func clearCutFromClick() {
        //: let config = ShowAlertConfig()
        let config = ManagerAlertConfig()
        //: config.alignment = .center
        config.alignment = .center

        //: TalkingAlertShow.customAlert(message: "Do you want to mark all messages as read?".localized, leftBtnTitle: "Cancel".localized, rightBtnTitle: "OK".localized, leftBlock: {
        VoiceAlertShow.customForTotal(message: String(bytes: str_frameMakeValue.map{broadcastIndex(constitutional: $0)}, encoding: .utf8)!.localized, leftBtnTitle: (String(str_viewData.suffix(6))).localized, rightBtnTitle: "OK".localized, leftBlock: {
            //: TalkingAlertShow.hideAlert()
            VoiceAlertShow.bias()

            //: }, rightBlock: {
        }, rightBlock: {
            //: guard self.manager.req_moreMsgGetUnredConversationCount() > 0 else { return }
            guard self.manager.speechSend() > 0 else { return }
            //: self.manager.moreMsgArr.forEach { model in
            self.manager.moreMsgArr.forEach { model in
                //: guard model.unreadCount > 0 else { return }
                guard model.unreadCount > 0 else { return }
                //: V2TIMManager.sharedInstance().markC2CMessage(asRead: model.userID) {
                V2TIMManager.sharedInstance().markC2CMessage(asRead: model.userID) {
                    //: } fail: { code, desc in
                } fail: { code, desc in
                    //: UploadLogTool.writeLog(msg: "MoreMessages一键已读失败：code:\(code), desc:\(String(describing: desc))")
                    QueryThen.bringInstance(msg: (String(str_sendTitle) + String(str_appearImageName.suffix(7))) + "\(code)" + (String(str_tabText) + String(str_frameData)) + "\(String(describing: desc))")
                }
            }

            //: }, config: config)
        }, config: config)
    }
}

// MARK: - 通知

//: extension TalkingMoreMessagesListViewController {
extension TotalUserDataSource {
    //: func func__installNotificationObservers() {
    func deed() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(func__getUserInfoDataFinish(notification:)),
                                               selector: #selector(equalityNotification(notification:)),
                                               //: name: USER_INFO_MSG_LIST_NOTIFICATION,
                                               name: kLet_buttonValue,
                                               //: object: nil)
                                               object: nil)

        // 更新消息列表用户在线状态
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(func__updateUserOnlineStatus(notif:)),
                                               selector: #selector(contentBelow(notif:)),
                                               //: name: MSGLIST_UPDATE_ONLINESTATUS_NOTIFICATION,
                                               name: kLet_likeValue,
                                               //: object: nil)
                                               object: nil)
    }

    //: @objc func func__getUserInfoDataFinish(notification: NSNotification) {
    @objc func equalityNotification(notification: NSNotification) {
        //: let dic = notification.object as? Dictionary<String, Any>
        let dic = notification.object as? [String: Any]
        //: for userID in dic!.keys {
        for userID in dic!.keys {
            //: let aInfoWrap: CoverThen = dic![userID] as! CoverThen
            let aInfoWrap: CoverThen = dic![userID] as! CoverThen
            //: let aModel: TalkingConversationModel? = EdificeV2Listener.shared.func__getCacheConversationModel(targetId: userID)
            let aModel: MediaConversationModel? = EdificeV2Listener.shared.pushPossibility(targetId: userID)
            //: if aModel != nil && aModel!.gj_userInfo != aInfoWrap {
            if aModel != nil, aModel!.gj_userInfo != aInfoWrap {
                //: aModel!.gj_userInfo = aInfoWrap
                aModel!.gj_userInfo = aInfoWrap
            }
        }
        //: setHaveLocalInmate()
        redirect()

        //: self.reloadData()
        self.informationPerform()
    }

    /// 音视频新通话新建立的会话，本地缓存有时，更新会话标识
    //: func setHaveLocalInmate() {
    func redirect() {
        //: let intimate: Dictionary<String, Any> = UserDefaults.standard.object(forKey: isConversationIntimateCacheKey) as? Dictionary<String, Any> ?? Dictionary.init()
        let intimate: [String: Any] = UserDefaults.standard.object(forKey: kLet_limitData) as? [String: Any] ?? Dictionary()
        //: for userID in intimate.keys {
        for userID in intimate.keys {
            //: let aModel: TalkingConversationModel? = EdificeV2Listener.shared.func__getCacheConversationModel(targetId: userID)
            let aModel: MediaConversationModel? = EdificeV2Listener.shared.pushPossibility(targetId: userID)
            //: if aModel != nil {
            if aModel != nil {
                //: aModel!.gj_userInfo?.isHaveSession = true
                aModel!.gj_userInfo?.isHaveSession = true
            }
        }
        //: UserDefaults.standard.set(Dictionary<String, Any>.init(), forKey: isConversationIntimateCacheKey)
        UserDefaults.standard.set([String: Any](), forKey: kLet_limitData)
    }

    /// 更新消息列表用户在线状态
    //: @objc private func func__updateUserOnlineStatus(notif: Notification) {
    @objc private func contentBelow(notif: Notification) {
        //: guard let userInfo = notif.userInfo else { return }
        guard let userInfo = notif.userInfo else { return }
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: if self.manager.update_userOnlineStatus(userInfo: userInfo as! [String: Any]) == true {
            if self.manager.changeIn(userInfo: userInfo as! [String: Any]) == true {
                //: self.tableView.reloadData()
                self.tableView.reloadData()
            }
        }
    }
}

// MARK: - UITableViewDelegate, UITableViewDataSource

//: extension TalkingMoreMessagesListViewController: UITableViewDelegate, UITableViewDataSource {
extension TotalUserDataSource: UITableViewDelegate, UITableViewDataSource {
    /// - UITableViewDataSource
    //: func numberOfSections(in tableView: UITableView) -> Int {
    func numberOfSections(in _: UITableView) -> Int {
        //: return 1
        return 1
    }

    //: func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
    func tableView(_: UITableView, numberOfRowsInSection _: Int) -> Int {
        //: return self.manager.moreMsgArr.count
        return self.manager.moreMsgArr.count
    }

    //: func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        //: let cell = tableView.dequeueReusableCell(withIdentifier: TalkingChatListTableCell.className(), for: indexPath) as! TalkingChatListTableCell
        let cell = tableView.dequeueReusableCell(withIdentifier: StandardMainViewCell.className(), for: indexPath) as! StandardMainViewCell
        //: var model = self.manager.req_moreMsgConversationModel(indexPath: indexPath)
        var model = self.manager.eigenvalueMargin(indexPath: indexPath)
        //: model = self.manager.getUserInfoFromCache(model: model)
        model = self.manager.pushModel(model: model)
        //: if !(model?.gj_userInfo?.tpAuth ?? false) {
        if !(model?.gj_userInfo?.tpAuth ?? false) {
            //: model?.gj_userInfo?.tpAuth = model?.ismoreAPAuto ?? false
            model?.gj_userInfo?.tpAuth = model?.ismoreAPAuto ?? false
        }
        //: cell.refreshCell(model: model)
        cell.discoView(model: model)

        //: return cell
        return cell
    }

    //: func tableView(_ tableView: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
    func tableView(_: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        //: guard let model = self.manager.req_moreMsgConversationModel(indexPath: indexPath) else {
        guard let model = self.manager.eigenvalueMargin(indexPath: indexPath) else {
            //: return false
            return false
        }
        //: guard model.chatType == .private else { return false }
        guard model.chatType == .private else { return false }

        //: return true
        return true
    }

    //: @available(iOS 11.0, *)
    @available(iOS 11.0, *)
    //: func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
    func tableView(_: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        //: guard let model = self.manager.req_moreMsgConversationModel(indexPath: indexPath) else { return nil }
        guard let model = self.manager.eigenvalueMargin(indexPath: indexPath) else { return nil }

        // 举报
        //: let reportBtn = UIContextualAction(style: .normal, title: nil) { action, view, completionHandler in
        let reportBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            //: self.reportChatListCell(conversationModel: model)
            self.chat(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let reportBtnTrashcan = UIImage.nearCounterval(name: "btn_message_report_nor").cgImage {
        if let reportBtnTrashcan = UIImage.nearCounterval(name: (String(str_nameData) + String(str_kitTitleText.prefix(6)))).cgImage {
            //: reportBtn.image = ImageWithoutRender(cgImage: reportBtnTrashcan, scale: UIScreen.main.nativeScale, orientation: .up)
            reportBtn.image = LabelWithoutRender(cgImage: reportBtnTrashcan, scale: UIScreen.main.nativeScale, orientation: .up)
        }
        //: reportBtn.backgroundColor = UIColor(hex: "#FF935D")
        reportBtn.backgroundColor = UIColor(hex: (String(str_dropData)))

        // 拉黑
        //: let blockBtn = UIContextualAction(style: .normal, title: nil) { action, view, completionHandler in
        let blockBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            //: self.blockChatListCell(conversationModel: model)
            self.swageBlockNextPossibility(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let blockBtnTrashcan = UIImage.nearCounterval(name: "btn_message_block_nor").cgImage {
        if let blockBtnTrashcan = UIImage.nearCounterval(name: (String(str_roundData) + str_addEmptyName.replacingOccurrences(of: "status", with: "o"))).cgImage {
            //: blockBtn.image = ImageWithoutRender(cgImage: blockBtnTrashcan, scale: UIScreen.main.nativeScale, orientation: .up)
            blockBtn.image = LabelWithoutRender(cgImage: blockBtnTrashcan, scale: UIScreen.main.nativeScale, orientation: .up)
        }
        //: blockBtn.backgroundColor = UIColor(hex: "#C179F9")
        blockBtn.backgroundColor = UIColor(hex: (String(str_showData.suffix(7))))

        // 删除
        //: let deleteBtn = UIContextualAction(style: .normal, title: nil) { action, view, completionHandler in
        let deleteBtn = UIContextualAction(style: .normal, title: nil) { _, _, completionHandler in
            //: self.deleteChatListCell(conversationModel: model)
            self.circleList(conversationModel: model)
            //: completionHandler(false)
            completionHandler(false)
        }
        //: if let deleteBtnTrashcan = UIImage.nearCounterval(name: "btn_message_delete_nor").cgImage {
        if let deleteBtnTrashcan = UIImage.nearCounterval(name: (String(str_localValue.prefix(6)) + "ssage_" + str_toName.replacingOccurrences(of: "shared", with: "el") + String(str_shareData))).cgImage {
            //: deleteBtn.image = ImageWithoutRender(cgImage: deleteBtnTrashcan, scale: UIScreen.main.nativeScale, orientation: .up)
            deleteBtn.image = LabelWithoutRender(cgImage: deleteBtnTrashcan, scale: UIScreen.main.nativeScale, orientation: .up)
        }
        //: deleteBtn.backgroundColor = UIColor(hex: "#FF506D")
        deleteBtn.backgroundColor = UIColor(hex: (str_labelTitle.replacingOccurrences(of: "equal", with: "#") + str_mValue.replacingOccurrences(of: "selected", with: "6D")))

        //: let actions = [deleteBtn, blockBtn, reportBtn]
        let actions = [deleteBtn, blockBtn, reportBtn]
        //: let config = UISwipeActionsConfiguration(actions: actions)
        let config = UISwipeActionsConfiguration(actions: actions)
        //: config.performsFirstActionWithFullSwipe = false
        config.performsFirstActionWithFullSwipe = false
        //: return config
        return config
    }

    /// - UITableViewDelegate
    //: func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
    func tableView(_: UITableView, heightForRowAt _: IndexPath) -> CGFloat {
        //: return 76.0
        return 76.0
    }

    //: func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
    func tableView(_: UITableView, heightForHeaderInSection _: Int) -> CGFloat {
        //: return 0.01
        return 0.01
    }

    //: func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
    func tableView(_: UITableView, heightForFooterInSection _: Int) -> CGFloat {
        //: return 0.01
        return 0.01
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        //: return UIView()
        return UIView()
    }

    //: func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForFooterInSection _: Int) -> UIView? {
        //: return UIView()
        return UIView()
    }

    //: func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        //: tableView.deselectRow(at: indexPath, animated: true)
        tableView.deselectRow(at: indexPath, animated: true)
        //: guard let model = self.manager.req_moreMsgConversationModel(indexPath: indexPath) else {
        guard let model = self.manager.eigenvalueMargin(indexPath: indexPath) else {
            //: return
            return
        }
        // 入口埋点
        //: uploadRecord.uploadRecordEvent(eventID: ClickUserChatNoP, toUid: model.targetId)
        kLet_voiceText.quantityingTime(eventID: kLet_statusValue, toUid: model.targetId)
        //: LeadingEqualReactiveCompatible.share.func__pushToPriveteChatVC(chatID: model.targetId) { vc in
        LeadingEqualReactiveCompatible.share.cellRowCompletion(chatID: model.targetId) { vc in
            //: if model.gj_userInfo != nil {
            if model.gj_userInfo != nil {
                //: vc.isHaveSession = model.gj_userInfo!.isHaveSession
                vc.isHaveSession = model.gj_userInfo!.isHaveSession
            }
        }
    }
}

// MARK: - TalkingTXIMDelegate

//: extension TalkingMoreMessagesListViewController: IMManagerDelegate {
extension TotalUserDataSource: ToObjectProtocol {
    //: func onRefreshConversationList(data: [TalkingConversationModel]) {
    func plan(data _: [MediaConversationModel]) {
        //: self.manager.req_moreMsgInitData()
        self.manager.totalerval()
        //: refreshTableView()
        willView()
    }
}

// MARK: - Layout

//: extension TalkingMoreMessagesListViewController {
extension TotalUserDataSource {
    /// UI
    //: private func createUI() {
    private func stopUser() {
        //: view.addSubview(tableView)
        view.addSubview(tableView)
        //: tableView.snp.makeConstraints { make in
        tableView.snp.makeConstraints { make in
            //: make.edges.equalToSuperview()
            make.edges.equalToSuperview()
        }

        //: self.view.addSubview(emptyView)
        self.view.addSubview(emptyView)
        //: emptyView.snp.makeConstraints { make in
        emptyView.snp.makeConstraints { make in
            //: make.edges.equalTo(tableView)
            make.edges.equalTo(tableView)
        }
        //: emptyView.emptyBlock = { [weak self] in
        emptyView.emptyBlock = { [weak self] in
            //: self?.reloadData()
            self?.informationPerform()
        }
    }
}
