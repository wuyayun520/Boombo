...
fileprivate let str_contentText:[UInt8] = [0x67,0x60,0x67,0x7a,0x26,0x6d,0x61,0x6a,0x6b,0x7c,0x34,0x27,0x2e,0x66,0x6f,0x7d,0x2e,0x60,0x61,0x7a,0x2e,0x6c,0x6b,0x6b,0x60,0x2e,0x67,0x63,0x7e,0x62,0x6b,0x63,0x6b,0x60,0x7a,0x6b,0x6a]

/*: "intimate_lock" :*/
fileprivate let str_progressContent:[Character] = ["i","n","t","i","m"]
fileprivate let str_neverValue:String = "ate_lockequal cell behind collection if"

/*: "Unlock" :*/
fileprivate let str_addBringContent:[Character] = ["U","n","l","o","c","k"]

/*: "nav_back_black_nor" :*/
fileprivate let str_errorStateName:String = "nav_total return show view model"
fileprivate let str_serverValue:[Character] = ["b","l","a","c","k","_","n","o","r"]

/*: "toUid" :*/
fileprivate let str_imageValue:String = "toUidshare status up tool let"

/*: "giftId" :*/
fileprivate let str_courseText:String = "center locationgiftId"

/*: "giftNum" :*/
fileprivate let str_makeText:String = "true let to selfgiftN"
fileprivate let str_valueTitle:String = "uicon"

/*: "unlockMsgId" :*/
fileprivate let str_viewData:[Character] = ["u","n","l","o","c","k"]
fileprivate let str_tableToolText:String = "MsgIdroom let for if have"

/*: "galleryId" :*/
fileprivate let str_pathData:String = "view kit countgalleryI"
fileprivate let str_toName:String = "fatal"

/*: "Load failed, return to retry" :*/
fileprivate let str_makeData:String = "video letLoad f"
fileprivate let str_headData:String = "d, rinmate content beneath"
fileprivate let str_equalName:String = "o retrylet if clean section index"

/*: "You will spend 1 chance to see the private video, you have  :*/
fileprivate let str_appName:[UInt8] = [0xf3,0xc5,0xdf,0x8a,0xdd,0xc3,0xc6,0xc6,0x8a,0xd9,0xda,0xcf,0xc4,0xce,0x8a,0x9b,0x8a,0xc9,0xc2,0xcb,0xc4,0xc9,0xcf,0x8a,0xde,0xc5,0x8a,0xd9,0xcf,0xcf,0x8a,0xde,0xc2,0xcf,0x8a,0xda,0xd8,0xc3,0xdc,0xcb,0xde,0xcf,0x8a,0xdc,0xc3,0xce,0xcf,0xc5,0x86,0x8a,0xd3,0xc5,0xdf,0x8a,0xc2,0xcb,0xdc,0xcf,0x8a]

private func selfIndicator(you num: UInt8) -> UInt8 {
    return num ^ 170
}

/*:  chance left." :*/
fileprivate let str_dataImageEqualValue:String = " chaon clear equal import self"
fileprivate let str_appContent:[Character] = ["n","c","e"," ","l","e","f","t","."]

/*: "icon_photo_lock" :*/
fileprivate let str_insideName:[Character] = ["i","c","o","n","_","p","h"]
fileprivate let str_clickData:String = "next location imageoto_lock"

/*: "Click on the screen to play this video(Burn After Reading)" :*/
fileprivate let str_pastData:[UInt8] = [0x29,0x67,0x6e,0x69,0x64,0x61,0x65,0x52,0x20,0x72,0x65,0x74,0x66,0x41,0x20,0x6e,0x72,0x75,0x42,0x28,0x6f,0x65,0x64,0x69,0x76,0x20,0x73,0x69,0x68,0x74,0x20,0x79,0x61,0x6c,0x70,0x20,0x6f,0x74,0x20,0x6e,0x65,0x65,0x72,0x63,0x73,0x20,0x65,0x68,0x74,0x20,0x6e,0x6f,0x20,0x6b,0x63,0x69,0x6c,0x43]

/*: "icon_photo_burn" :*/
fileprivate let str_cellData:String = "icon_pbutton exception path title make"

/*: "Already burned" :*/
fileprivate let str_cameraRedTitle:[Character] = ["A","l","r","e","a","d","y"," "]
fileprivate let str_eventText:String = "BURNED"
...
// __CLOSE_PRINT__
//
//  ManagerTotalimatePreviewVideoViewDelegate.swift
//  AbroadTalking
//
...
//: typealias IntimateVideoBlock = (_ model: ValueJsonModel) -> Void
typealias IntimateVideoBlock = (_ model: ValueJsonModel) -> Void
//: typealias VideoStatusBlock = (_ status: VideoPlayerStatus) -> Void
typealias VideoStatusBlock = (_ status: TotalPlayerStatus) -> Void

//: class TalkingIntimateVideoPreviewVC: TalkingBaseViewController {
class ManagerTotalimatePreviewVideoViewDelegate: NameViewController {
    ...
    //: private var videoTotalData = DemonstrateCellData(direction: .MsgDirectionIncoming)
    private var videoTotalData = DemonstrateCellData(direction: .MsgDirectionIncoming) // 视频消息数据
    //: private var videoModel = ValueJsonModel()
    private var videoModel = ValueJsonModel() // 当前私密视频模型
    ...
    ///   - videoCellData: 视频模型
    ///   - toUid: 对方Id
    //: init(videoCellData: DemonstrateCellData, toUid: String, preNum: Int) {
    init(videoCellData: DemonstrateCellData, toUid: String, preNum: Int) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (videoTotalData.msgModel.user.uid == Int(WithAdministratorAppManager.share.loginUserMode.userID))
        isMySelf = (videoTotalData.msgModel.user.uid == Int(WithAdministratorAppManager.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_contentText.map{$0^14}, encoding: .utf8)!)
    }
    ...
    override func loadView() {
        ...
        if #available(iOS 13.2, *) {
            ...
            if WithAdministratorAppManager.share.appConfigMode.disableShootScreen {
                ...
                //: let bgView = MAScreenShieldView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight-kLet_likeTitle))
                let bgView = CoverShieldView(frame: CGRect(x: 0, y: 0, width: kLet_senseContent, height: kLet_environmentClickValue - kLet_likeTitle))
                //: self.view = bgView
                self.view = bgView
            }
            //: } else {
    ...
    private lazy var videoProgressView: HandleView = {
    ...
    private lazy var giftBgView: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.nearCounterval(name: "intimate_lock")
        img.image = UIImage.nearCounterval(name: (String(str_progressContent) + String(str_neverValue.prefix(8))))
        //: return img
        return img
        //: }()
    }()
...
//    }()
    ...
    private lazy var desLab: UILabel = {
        ...
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.font = UIFont.behindUpRoom(fontSize: 19)
        lab.font = UIFont.behindUpRoom(fontSize: 19)
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(str_addBringContent)).localized, for: .normal)
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.liveAttention(type: .Medium, fontSize: 19)
        btn.titleLabel?.font = UIFont.liveAttention(type: .Medium, fontSize: 19)
        ...
        btn.addTarget(self, action: #selector(sendPan), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var progressView: StartStackView = {
        ...
        let frame = CGRect(x: kLet_senseContent - 42 - 15, y: kLet_messageContent + 10, width: 42, height: 42)
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.nearCounterval(name: (String(str_errorStateName.prefix(4)) + "back_" + String(str_serverValue))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(bodyRouteSnap), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ManagerTotalimatePreviewVideoViewDelegate {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func messageStart(giftId: String, giftNum: String) {
        ...
        params[(String(str_imageValue.prefix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(str_courseText.suffix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(str_makeText.suffix(5)) + str_valueTitle.replacingOccurrences(of: "icon", with: "m"))] = giftNum
        //: params["unlockMsgId"] = self.videoTotalData.msgModel.msgInfo.msgId
        params[(String(str_viewData) + String(str_tableToolText.prefix(5)))] = self.videoTotalData.msgModel.msgInfo.msgId
        //: params["galleryId"] = self.videoModel.galleryId
        params[(String(str_pathData.suffix(8)) + str_toName.replacingOccurrences(of: "fatal", with: "d"))] = self.videoModel.galleryId

        // 送礼接口
        //: BridgeReactiveCompatible.req_SendGiftPrivete(param: params, completion: { succeed, result, errorModel in
        BridgeReactiveCompatible.sendToCompletionSearched(param: params, completion: { succeed, result, errorModel in
            //: if succeed {
...
extension ManagerTotalimatePreviewVideoViewDelegate {
    ...
    @objc private func bodyRouteSnap() {
    ...
    @objc private func sendPan() {
        //: req_sendGiftMsg(giftId: videoModel.giftId, giftNum: "1")
        messageStart(giftId: videoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnVideoTapGesture() {
    @objc private func choice() {
        //: guard WithAdministratorAppManager.share.networkStatus != .Unavailable else {
        guard WithAdministratorAppManager.share.networkStatus != .Unavailable else {
            //: func__showStatusBarErrorMsg(showMsg: kNetErrorMsg)
            atShow(showMsg: kLet_detailMessageValue)
            //: return
        ...
        StandardProgressHUD.aEnablee(superView: self.view)
        ...
        self.videoStatusBlock = { [weak self] status in
            ...
            guard status == .Playing else {
                ...
                if status == .Failed {
                    ...
                    self.atShow(showMsg: (String(str_makeData.suffix(6)) + "aile" + String(str_headData.prefix(4)) + "eturn t" + String(str_equalName.prefix(7))).localized)
                }
                //: return
                return
            }
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.timeObserver(totalTime: 600, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension ManagerTotalimatePreviewVideoViewDelegate {
    ...
    @objc private func pathPosition() {
...
extension ManagerTotalimatePreviewVideoViewDelegate {
    ...
    private func depilate(urlStr: String, isMosaic: Bool) {
        ...
        guard isMySelf == false else { // 本人发的图片，展示本地缓存资源
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.imageView.image = UIImage.error(image, size: 90)
                self.imageView.image = UIImage.error(image, size: 90)
                //: } else {
            } else {
        ...
        imageView.oreoCookieWhiteCompletion(urlStr: urlStr) { [weak self] image in
            //: guard let self = self else { return }
            guard let self = self else { return }
    ...
    private func timeEnable() {
        ...
        self.player?.drunk(url: videoPath, view: self.videoView)
    }
}

// MARK: - MainReactiveCompatible, ModelThen

//: extension TalkingIntimateVideoPreviewVC: TalkingVideoPlayerDelegate, InteractionViewDelegate {
extension ManagerTotalimatePreviewVideoViewDelegate: MainReactiveCompatible, ModelThen {
    ...
    func poolPlayerReport(player _: ButtonPlayerManager, status: TotalPlayerStatus) {
        ...
        self.videoProgressView.cellStatus(status: status)
        //: if status == .Playing {
        if status == .Playing {
    ...
    func informant(player _: ButtonPlayerManager, duration: Int, currentTime: Int) {
        ...
        self.videoProgressView.likeDismiss(duration: duration, currentTime: currentTime)
    }
    ...
    func showSub(player _: ButtonPlayerManager, progress: CGFloat) {
        ...
        self.videoProgressView.haveInMind(value: progress)
    }
...
//        giftBgView.addSubview(giftIcon)
    ...
    private func part() {
        ...
        videoProgressView.snp.makeConstraints { make in
            ...
            make.leading.trailing.equalToSuperview()
            //: make.top.equalToSuperview().offset(kLet_likeTitle+30)
            make.top.equalToSuperview().offset(kLet_likeTitle + 30)
            //: make.bottom.equalToSuperview().offset(-(kLet_appMarginText+64))
            make.bottom.equalToSuperview().offset(-(kLet_appMarginText + 64))
        }
        ...
        desLab.snp.makeConstraints { make in
            ...
            make.centerX.equalToSuperview()
            //: make.centerY.equalToSuperview().offset(kLet_likeTitle/2)
            make.centerY.equalToSuperview().offset(kLet_likeTitle / 2)
        }
...
//        }
        ...
        progressView.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(kLet_senseContent - 42 - 15)
    ...
    private func tabCleaveTip() {
        // 添加接收到音视频通话通知
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(startTalking),
                                               selector: #selector(pathPosition),
    ...
    private func ta() {
        ...
        if videoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.depilate(urlStr: videoModel.coverImg, isMosaic: true)
            ...
            self.videoProgressView.likeDismiss(duration: videoModel.duration, currentTime: 0)
...
//            self.desLab.text = "Need a %@ (%@ Gold coins) to unlock".localizedArguments(videoModel.giftName, videoModel.giftPrice)
            ...
            self.desLab.text = String(bytes: str_appName.map{selfIndicator(you: $0)}, encoding: .utf8)! + "\(self.previewNum)" + (String(str_dataImageEqualValue.prefix(4)) + String(str_appContent))
            ...
            if videoModel.burnStatus == 0 { // 未销毁
                ...
                //: self.videoProgressView.updateProgressDurationAndCurrentTime(duration: videoModel.duration, currentTime: 0)
                self.videoProgressView.likeDismiss(duration: videoModel.duration, currentTime: 0)
                //: self.intimateStatusImgV.image = UIImage.nearCounterval(name: "icon_photo_lock")
                self.intimateStatusImgV.image = UIImage.nearCounterval(name: (String(str_insideName) + String(str_clickData.suffix(8))))
                //: self.desLab.text = "Click on the screen to play this video(Burn After Reading)".localized
                self.desLab.text = String(bytes: str_pastData.reversed(), encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnVideoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(choice))
                //: self.maskView.addGestureRecognizer(tap)
                self.maskView.addGestureRecognizer(tap)

                //: } else {
            } else {
                //: self.intimateStatusImgV.image = UIImage.nearCounterval(name: "icon_photo_burn")
                self.intimateStatusImgV.image = UIImage.nearCounterval(name: (String(str_cellData.prefix(6)) + "hoto_burn"))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(str_cameraRedTitle) + str_eventText.lowercased()).localized
            }
