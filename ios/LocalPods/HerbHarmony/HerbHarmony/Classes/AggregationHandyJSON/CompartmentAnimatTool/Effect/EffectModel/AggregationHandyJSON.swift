...
/*: "id" :*/
fileprivate let str_mName:[Character] = ["i","d"]

/*: "num" :*/
fileprivate let str_hiddenName:[Character] = ["n","u","m"]
...
@objc public enum ExamineVarArg: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct AggregationHandyJSON: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func tagTemp(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct ContainerAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(str_hiddenName))
    }
}
