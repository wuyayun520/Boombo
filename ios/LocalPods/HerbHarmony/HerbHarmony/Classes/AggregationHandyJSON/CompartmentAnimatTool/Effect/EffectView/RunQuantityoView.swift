...
/*: "%@.mp4" :*/
fileprivate let str_targetData:String = "info target self view make%@.mp4"

/*: "http" :*/
fileprivate let str_arrayContent:String = "hdistancep"
...
class RunQuantityoView: MessageItemView {
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(searchStub))
    ...
    override func makeAnimating() {
        ...
        let path = DataThen.shared.toUp(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? StartMeasurable())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(str_targetData.suffix(6))), path)
    ...
    @objc func searchStub() {}
}

///  --- HWDMP4PlayDelegate

//: extension TalkingVapAnimationView: HWDMP4PlayDelegate {
extension RunQuantityoView: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((str_arrayContent.replacingOccurrences(of: "distance", with: "tt"))) {
