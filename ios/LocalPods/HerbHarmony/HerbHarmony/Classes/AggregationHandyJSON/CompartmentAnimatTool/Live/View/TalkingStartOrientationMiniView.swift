...
/*: "99+" :*/
fileprivate let str_backValue:String = "99+"

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_managerValue:[UInt8] = [0x55,0x52,0x55,0x48,0x14,0x5f,0x53,0x58,0x59,0x4e,0x6,0x15,0x1c,0x54,0x5d,0x4f,0x1c,0x52,0x53,0x48,0x1c,0x5e,0x59,0x59,0x52,0x1c,0x55,0x51,0x4c,0x50,0x59,0x51,0x59,0x52,0x48,0x59,0x58]

/*: "#2DF2FF" :*/
fileprivate let str_frameData:String = "model for make model add#2DF2"
fileprivate let str_contentName:[Character] = ["F","F"]

/*: "Live" :*/
fileprivate let str_mainName:String = "string operation contentLive"

/*: "img_home_shadow_icon" :*/
fileprivate let str_pagePathTitle:[Character] = ["i","m","g","_","h","o","m","e","_","s","h","a","d","o"]
fileprivate let str_colorTimeValue:String = "w_iconin return first"

/*: "get json error" :*/
fileprivate let str_dataName:String = "get let equal result gift"
fileprivate let str_dateData:String = "rpathr"

/*: "#FF2348" :*/
fileprivate let str_liveName:[Character] = ["#","F","F","2","3","4","8"]

/*: "#35C759" :*/
fileprivate let str_toValue:String = "#"
fileprivate let str_collectionName:String = "cell number35C759"

/*: "mfBean" :*/
fileprivate let str_topContent:String = "mfBeankey var index make"

/*: "%.1f" :*/
fileprivate let str_normalCellValue:[Character] = ["%",".","1","f"]

/*: "points\n≈ $" :*/
fileprivate let str_sharedTitle:[Character] = ["p","o","i","n","t","s"]
fileprivate let str_matterValue:[Character] = ["\n","≈"," ","$"]

/*:   :*/
fileprivate let str_ofViewValue:String = "click"
...
// __CLOSE_PRINT__
//
//  TalkingStartOrientationMiniView.swift
//  AbroadTalking
//
...
public var kLet_homeData: Int {
    //: return WithAdministratorAppManager.share.appConfigMode.countdown1 + WithAdministratorAppManager.share.appConfigMode.countdown2
    return WithAdministratorAppManager.share.appConfigMode.countdown1 + WithAdministratorAppManager.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class TalkingStartOrientationMiniView: ShowView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= WithAdministratorAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(kLet_homeData - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                sight()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(WithAdministratorAppManager.share.loginUserMode.mf_bean) > 0 {
        if Double(WithAdministratorAppManager.share.loginUserMode.mf_bean) > 0 {
            //: setPointText(mfBean: 0.0)
            startPost(mfBean: 0.0)
        }
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_managerValue.map{$0^60}, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: RoomThen = {
        ...
        let image = UIImage.apply(color: UIColor(hex: (String(str_frameData.suffix(5)) + String(str_contentName)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.smart(), for: .normal)
        img.setImage(image.smart(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(str_mainName.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.nearCounterval(name: (String(str_pagePathTitle) + String(str_colorTimeValue.prefix(6))))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(str_dataName.prefix(4)) + "json er" + str_dateData.replacingOccurrences(of: "path", with: "o")))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.font = UIFont.behindUpRoom(fontSize: 20)
        lab.font = UIFont.behindUpRoom(fontSize: 20)
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.textColor = UIColor(hex: "#FF2348")
        lab.textColor = UIColor(hex: (String(str_liveName)))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (str_toValue.capitalized + String(str_collectionName.suffix(6))))
...
extension TalkingStartOrientationMiniView {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func viewOf() -> TalkingStartOrientationMiniView {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = TalkingStartOrientationMiniView(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = DataSocketDelegate.shared.liveMiniPosition
    ...
    func sight() {
        ...
        liveStatusImgV.isHidden = !SectionPushListener.cover().isLive
...
extension TalkingStartOrientationMiniView {
    ...
    private func videoName() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(pointchange(notification:)),
                                               selector: #selector(resumeBy(notification:)),
                                               //: name: LIVE_USER_POINT_CHANGE,
                                               name: kLet_conversationName,
                                               //: object: nil)
                                               object: nil)
    }

    //: @objc private func pointchange(notification: NSNotification) {
    @objc private func resumeBy(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(str_topContent.prefix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        startPost(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func startPost(mfBean: Double) {
        //: let nowBean = WithAdministratorAppManager.share.loginUserMode.mf_bean + mfBean
        let nowBean = WithAdministratorAppManager.share.loginUserMode.mf_bean + mfBean
        //: WithAdministratorAppManager.share.loginUserMode.mf_bean = nowBean
        WithAdministratorAppManager.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((String(str_sharedTitle) + String(str_matterValue)).localized) \(divisor)"
