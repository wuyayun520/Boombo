...
/*: "#ffffff" :*/
fileprivate let str_checkTitle:String = "#ffffffuser height distance if"

/*: "/dist/loungePlus/index.html" :*/
fileprivate let str_localName:String = "constraint to/dis"
fileprivate let str_videoContent:String = "head viewngePl"
fileprivate let str_makeValue:String = "pop let label var viewex.html"

/*: "https://" :*/
fileprivate let str_appTitle:String = "pic show for requesthttps://"

/*: "http://" :*/
fileprivate let str_customValue:[Character] = ["h"]
fileprivate let str_kindMakeValue:String = "view fillttp://"
...
class PartThen: UIView {
    ...
    let Broadcast_TextColor = (String(str_checkTitle.prefix(7)))
    //: let Broadcast_ContentFont: UIFont = UIFont.behindUpRoom(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .behindUpRoom(fontSize: 12)
    ...
    convenience init(model: SignScreenModel?) {
        //: self.init()
        self.init()
    ...
    private func withInsert() {
        ...
        if danumuMsgModel.headPic.count > 0 {
            ...
            headAnimatedImageView.nearSquare(urlStr: danumuMsgModel.headPic)
        }
        //: if danumuMsgModel.bgPic.count > 0 {
        if danumuMsgModel.bgPic.count > 0 {
            //: updateBackground(withUrl: danumuMsgModel.bgPic)
            heritageGroup(withUrl: danumuMsgModel.bgPic)
        }
    ...
    func heritageGroup(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.commentGesture(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.beforeCustom(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: SignScreenModel? {
        //: didSet {
        didSet {
            //: configModel()
            withInsert()
    ...
    var totalTime: CGFloat {
        ...
        return (kLet_senseContent + widthValue) * Broadcast_SpendTime / kLet_senseContent
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / kLet_senseContent
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / kLet_senseContent
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(carve(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        //: let l = UILabel()
        let l = UILabel()
        //: l.font = UIFont.behindUpRoom(fontSize: 12)
        l.font = UIFont.behindUpRoom(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
...
extension PartThen {
    ...
    @objc func carve(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: LeadingEqualReactiveCompatible.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            LeadingEqualReactiveCompatible.share.bindDownFrom(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: LeadingEqualReactiveCompatible.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                LeadingEqualReactiveCompatible.share.artefactType(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(str_localName.suffix(4)) + "t/lou" + String(str_videoContent.suffix(5)) + "us/ind" + String(str_makeValue.suffix(7)))) {
                    //: LeadingEqualReactiveCompatible.share.func__pushToSubscribePageWebVC()
                    LeadingEqualReactiveCompatible.share.numberimateVc()
                    //: return
                    return
                }
                //: let httpsUrl = WithAdministratorAppManager.share.appConfigMode.urlH5Domain+url
                let httpsUrl = WithAdministratorAppManager.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(str_appTitle.suffix(8))), with: (String(str_customValue) + String(str_kindMakeValue.suffix(6))))
                //: LeadingEqualReactiveCompatible.share.func__pushToWebVC(urlStr: httpUrl)
                LeadingEqualReactiveCompatible.share.loadWith(urlStr: httpUrl)
            }
