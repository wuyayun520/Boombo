...
fileprivate let str_giftCenterData:[UInt8] = [0xc0,0xc5,0xc0,0xcb,0x7f,0xba,0xc6,0xbb,0xbc,0xc9,0x91,0x80,0x77,0xbf,0xb8,0xca,0x77,0xc5,0xc6,0xcb,0x77,0xb9,0xbc,0xbc,0xc5,0x77,0xc0,0xc4,0xc7,0xc3,0xbc,0xc4,0xbc,0xc5,0xcb,0xbc,0xbb]

fileprivate func segmentCell(user num: UInt8) -> UInt8 {
...
/*: "move" :*/
fileprivate let str_greetText:String = "movmenu"
...
class TouchThen: UIView {
    ...
    var danumuMsgArray: [SignScreenModel] = []
    ...
    let TapAction: PublishSubject<SignScreenModel?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_giftCenterData.map{segmentCell(user: $0)}, encoding: .utf8)!)
    }
    ...
    func toReceive(_ danumuMsgModel: SignScreenModel?) {
    ...
    @objc func colorConversation(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(colorConversation(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension TouchThen: UIGestureRecognizerDelegate {
    ...
    func begin(for danumuMsgModel: SignScreenModel?, isFirst: Bool) {
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (str_greetText.replacingOccurrences(of: "menu", with: "e")))
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + itemView.totalTime) { [unowned self] in
                ...
                popTransmissionRowModelto(isFirst)
                //: })
            }
    ...
    func popTransmissionRowModelto(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                begin(for: model, isFirst: isFirst)
...
extension TouchThen {
    ...
    private func dismissSize() {
        ...
        firstChannelEmptyBlock = { [unowned self] in
            ...
            popTransmissionRowModelto(true)
        ...
        secondChannelEmptyBlock = { [self] in
            ...
            popTransmissionRowModelto(false)
