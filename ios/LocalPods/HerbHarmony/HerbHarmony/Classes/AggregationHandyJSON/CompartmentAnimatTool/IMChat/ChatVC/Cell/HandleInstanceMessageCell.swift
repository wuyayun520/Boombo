...
fileprivate let str_pathTitle:[UInt8] = [0xf8,0xff,0xf8,0xe5,0xb9,0xf2,0xfe,0xf5,0xf4,0xe3,0xab,0xb8,0xb1,0xf9,0xf0,0xe2,0xb1,0xff,0xfe,0xe5,0xb1,0xf3,0xf4,0xf4,0xff,0xb1,0xf8,0xfc,0xe1,0xfd,0xf4,0xfc,0xf4,0xff,0xe5,0xf4,0xf5]

private func cardMake(to num: UInt8) -> UInt8 {
    return num ^ 145
}

/*: "icon_chat_miss" :*/
fileprivate let str_levelContent:String = "ICON"
fileprivate let str_withMVarContent:[Character] = ["t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let str_numberValue:String = "icon_chsucceed count add"
fileprivate let str_rawData:String = "type saveat_yes"
...
// __CLOSE_PRINT__
//
//  HandleInstanceMessageCell.swift
//  AbroadTalking
//
...
class HandleInstanceMessageCell: MTouchMsgCell {
//    var transBlock: (() -> Void)?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_pathTitle.map{cardMake(to: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var quoteView: QuantityryThen = {
        //: let quoteView = TalkingMsgQuoteView()
        let quoteView = QuantityryThen()
        //: quoteView.isHidden = false
        quoteView.isHidden = false
        //: quoteView.addTarget(self, action: #selector(quoteBtnClick), for: .touchUpInside)
        quoteView.addTarget(self, action: #selector(ingeminate), for: .touchUpInside)
        //: return quoteView
        return quoteView
        //: }()
    }()
...
extension HandleInstanceMessageCell {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)

        //: if let temTextData = data as? CollectionThen {
        if let temTextData = data as? CollectionThen {
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != IconThen.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.of() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.nearCounterval(name: (str_levelContent.lowercased() + "_cha" + String(str_withMVarContent)))
                ...
                self.transStatusImagV.image = UIImage.nearCounterval(name: (String(str_numberValue.prefix(7)) + String(str_rawData.suffix(6))))
            }
    ...
    func pathEraseGes() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(selectContent(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == IconThen.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == IconThen.finishBy())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            doingGes()
