...
+ (TUIMessageCellData *)insertTXMessageModelWithExtral:(NSDictionary *)extral
    ...
    if (data != nil) {
        [[EdificeV2Listener shared] pathWithTargetId:toUid msgCellData:data isPrivete:YES];
    }
    return data;
}
...
+ (TUIMessageCellData *)handleTXMessageSendModelWithExtral:(NSDictionary *)extral
    ...
    if (isTip) {
        ...
        DataNameModel *model = [[DataNameModel alloc]init];
        PathJsonModel *tipModel = [PathJsonModel modelWithDic:extralDict];
        model.tips = tipModel;
        data.msgModel = model;
        return data;
    }
    ...
    tempDict[@"user"] = @{
        @"uid":WithAdministratorAppManager.share.loginUserMode.userID,
        @"nickname":WithAdministratorAppManager.share.loginUserMode.nickname,
        @"headPic":WithAdministratorAppManager.share.loginUserMode.headPic,
        @"age":WithAdministratorAppManager.share.loginUserMode.age
    };
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"gift"]) {
        ...
        if ([msgInfo[@"isGroup"] isEqualToString:@"1"]) { // 群聊
            data = [[LanthanideCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        } else {
            data = [[ButtonCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        }
        
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        DataNameModel *model = [DataNameModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"WantGift"]) {
        TUIMessageCellData *data = nil;
        data = [[QuantityCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        DataNameModel *model = [DataNameModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"txt"]) {
        TUIMessageCellData *data = nil;
        if ([msgInfo[@"isGroup"] isEqualToString:@"1"]) { // 群聊
            data = [[DirectorThen alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        } else {
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"img"]) {
        IndexClearThen *data = [[IndexClearThen alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        DataNameModel *model = [DataNameModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"video"]) {
        DemonstrateCellData *data = [[DemonstrateCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        DataNameModel *model = [DataNameModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"audio"]) {
        VoxCellData *data = [[VoxCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        DataNameModel *model = [DataNameModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if (extral[@"gift"] != nil && [msgInfo[@"messageType"] integerValue] == 4) {
        CollectionThen *data = [[CollectionThen alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        DataNameModel *model = [DataNameModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"map"]) {
        TUIMessageCellData *data = nil;
        data = [[UserCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        DataNameModel *model = [DataNameModel modelWithDic:extral];
        data.msgModel = model;
...
+ (TUIMessageCellData *)handleTXMessageReSendModelWithData:(TUIMessageCellData *)cellData {
    // 图片
    if ([cellData isKindOfClass:[IndexClearThen class]]) {
        IndexClearThen *reCellData = (IndexClearThen *)cellData;
        NSArray *path = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
        NSString *filePath = [path objectAtIndex:0];
        filePath = [filePath stringByAppendingPathComponent:[IconThen eventIn]];
    ...
    // 视频
    if ([cellData isKindOfClass:[DemonstrateCellData class]]) {
        DemonstrateCellData *reCellData = (DemonstrateCellData *)cellData;
    ...
    // 语音
    if ([cellData isKindOfClass:[VoxCellData class]]) {
        VoxCellData *reCellData = (VoxCellData *)cellData;
        MarkThen *voiceModel = [ChiefThen thirdMessageWithMessage:reCellData.innerMessage];
        NSMutableDictionary * dic = [NSMutableDictionary dictionary];
        ...
        [msgInfo setObject:WithAdministratorAppManager.share.loginUserMode.userID forKey:@"senderId"];
        [dic setObject:msgInfo forKey:@"msgInfo"];
    ...
    // 文本
    if ([cellData isKindOfClass:[CollectionThen class]]) {
        CollectionThen *reCellData = (CollectionThen *)cellData;
        NSMutableDictionary *dic = [NSMutableDictionary dictionary];
        NSMutableDictionary *params = [NSMutableDictionary dictionary];
        {
            NSMutableDictionary * msgInfo = [NSMutableDictionary dictionary];
            ...
            if (reCellData.msgModel.quoteMsgInfo.of) { // 引用消息重发
...
+ (TUIMessageCellData *)handleTXReceiveMessageModelWithExtral:(NSDictionary*)dic
    ...
    if ([user[@"id"] integerValue] != WithAdministratorAppManager.share.loginUserMode.userID.integerValue) {
        uid = user[@"id"];
    }
    
    DataNameModel *model = [DataNameModel modelWithDic:dic[@"extra"]];
    BOOL isSelf = (model.user.uid == WithAdministratorAppManager.share.loginUserMode.userID.integerValue);
    
    //后台代发送的自动打招呼消息
    BOOL isAutoSendMsg = NO;
    if (model.msgInfo.uid.integerValue == WithAdministratorAppManager.share.loginUserMode.userID.integerValue && model.msgInfo.messageType == 3) {
        isSelf = YES;
        isAutoSendMsg = YES;
    }
    ...
    if ([dic[@"extra"][@"msgType"] isEqualToString:@"system"] || contenType == TXMessageContentType_tips) {
        DataNameModel *model = [[DataNameModel alloc]init];
        PathJsonModel *tipModel = [PathJsonModel modelWithDic:dic[@"extra"]];
    ...
    if (contenType == TXMessageContentType_picture) {
        cellData = [[IndexClearThen alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        
    } else if (contenType == TXMessageContentType_video) {
        cellData = [[DemonstrateCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        
    } else if(contenType == TXMessageContentType_gift) {
        if (tempModel.groupID != nil && tempModel.groupID.length > 0) { // 群聊gift
            cellData = [[LanthanideCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        } else { // 私聊gift
            cellData = [[ButtonCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        }
        ...
        cellData = [[QuantityCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        if (msgInfo.allKeys.count > 0) {
            model.msgInfo = [LikeModel modelWithDic:msgInfo];
        }
        cellData.msgModel = model;
        
    } else if (contenType == TXMessageContentType_voice) {
        cellData = [[VoxCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        VoxCellData *audioCellData = (VoxCellData *)cellData;
        if (audioCellData.voiceModel.db_voiceCacheWrap.msgId.length == 0) {
            ...
            MarkThen *cacheWrap = [MarkThen findWith:mainKey];
            if (!cacheWrap) {
                MarkThen *wrap = [MarkThen inserSmall:[TUISocialChatManager packageDictWithTXMessage:tempModel]];
        ...
        cellData = [[ClickThen alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        ...
        if ([dic[@"extra"][@"title"] length] > 0 ||
            ...
            cellData = [[AtonicityThen alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
            cellData.extraDict = dic[@"extra"];
            
        } else if (tempModel.groupID != nil && tempModel.groupID.length > 0) { // 群聊text
            cellData = [[DirectorThen alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        } else { // 私聊text
    ...
    if (model.user == nil) {
        ...
        CenterUserModel *user = [CenterUserModel modelWithDic:userDict];
        ...
        if (isAutoSendMsg) {
            ...
            user.headPic = WithAdministratorAppManager.share.loginUserMode.headPic;
        }
        model.user = user;
    }
    if (model.msgInfo == nil) {
        NSMutableDictionary *extraDict = [NSMutableDictionary dictionaryWithDictionary:dic[@"extra"]];
        LikeModel *msgInfo = [[LikeModel alloc]init];
    ...
    if(contenType == TXMessageContentType_gift && tempModel.groupID != nil && tempModel.groupID.length > 0) {
        if (model.toUser == nil) { // 送礼所有人
            StyleJsonModel *toUserInfo = [[StyleJsonModel alloc]init];
            toUserInfo.uid = @"all";
    ...
    // 私密照片Cache
    if (contenType == TXMessageContentType_picture) {
        NSDictionary *dict = [HypothesisModel userSize];
        if ([dict objectForKey:tempModel.msgID]) {
            cellData.msgModel.msgInfo.intimatePhoto = [[HypothesisModel alloc] initWithString:dict[tempModel.msgID] error:nil];
        }
    }
    
    // 私密视频Cache
    if (contenType == TXMessageContentType_video) {
        NSDictionary *dict = [ValueJsonModel origin];
        if ([dict objectForKey:tempModel.msgID]) {
            cellData.msgModel.msgInfo.video = [[ValueJsonModel alloc] initWithString:dict[tempModel.msgID] error:nil];
        }
    }
    
    // 索要礼物Cache
    if (contenType == TXMessageContentType_askforGift) {
        NSArray *arr = [TelecastingModel info];
...
+ (TXMessageContentType )messageCellType:(NSDictionary *)extraDic {
    ...
    DataNameModel *model = [DataNameModel modelWithDic:dic];
...
+ (NSDictionary *)packageDictWithTXMessage:(V2TIMMessage *)message {
    ...
    //自己发送的和接收的数据结构不一样
    if ([senderId isEqualToString:WithAdministratorAppManager.share.loginUserMode.userID]) {
    ...
    info[@"isRead"] = [NSString stringWithFormat:@"%d",[senderId isEqualToString:WithAdministratorAppManager.share.loginUserMode.userID]];
    return [info copy];
}
