...
fileprivate let str_touchDataText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_translate_nor" :*/
fileprivate let str_voiceValue:[Character] = ["i","c","o","n","_","c"]
fileprivate let str_plainName:String = "fill"
fileprivate let str_managerName:String = "make area fileat_t"
fileprivate let str_stackModeCurrentName:String = "ate_norlocation height model name back"

/*: "icon_chat_translate_pre" :*/
fileprivate let str_nameData:String = "icon_cif random value view if"
fileprivate let str_withName:String = "late_preobject in size play return"

/*: "icon_chat_miss" :*/
fileprivate let str_managerExpressContent:String = "request"
fileprivate let str_equalName:[Character] = ["c","o","n","_","c","h","a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let str_resultName:String = "icon_ccolor extension let"
fileprivate let str_youValue:String = "color"
...
class ButtonSendThen: MTouchMsgCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: DirectorThen?
    var textData: DirectorThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_touchDataText.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        //: label.numberOfLines = 0
        label.numberOfLines = 0
        //: label.font = UIFont.unprecedented(fontSize: 16)
        label.font = UIFont.unprecedented(fontSize: 16)
        //: return label
        return label
        //: }()
    ...
    lazy var transBtn: UIButton = {
        ...
        let btn = UIButton()
        //: btn.setImage(UIImage.nearCounterval(name: "icon_chat_translate_nor"), for: .normal)
        btn.setImage(UIImage.nearCounterval(name: (String(str_voiceValue) + str_plainName.replacingOccurrences(of: "fill", with: "h") + String(str_managerName.suffix(4)) + "ransl" + String(str_stackModeCurrentName.prefix(7)))), for: .normal)
        //: btn.setImage(UIImage.nearCounterval(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.nearCounterval(name: (String(str_nameData.prefix(6)) + "hat_trans" + String(str_withName.prefix(8)))), for: .highlighted)
...
extension ButtonSendThen {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)
        //: textData = data as? DirectorThen
        textData = data as? DirectorThen
        ...
        if imgPreview.count > 0, textData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
            ...
            giftImagV.nearSquare(urlStr: imgPreview)
        }
        ...
        //: let maxWidth = IconThen.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = IconThen.atWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transShowView.stopAnimating()
            //: self.transStatusImagV.image = UIImage.nearCounterval(name: "icon_chat_miss")
            self.transStatusImagV.image = UIImage.nearCounterval(name: (str_managerExpressContent.replacingOccurrences(of: "request", with: "i") + String(str_equalName)))
            //: } else if textData.translateStatus == .MsgTranslate_succeed {
        } else if textData.translateStatus == .MsgTranslate_succeed {
            //: self.transShowView.stopAnimating()
            self.transShowView.stopAnimating()
            //: self.transStatusImagV.image = UIImage.nearCounterval(name: "icon_chat_yes")
            self.transStatusImagV.image = UIImage.nearCounterval(name: (String(str_resultName.prefix(6)) + "hat_ye" + str_youValue.replacingOccurrences(of: "color", with: "s")))
        }
    ...
    func userColor() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(panTap(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func panTap(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
