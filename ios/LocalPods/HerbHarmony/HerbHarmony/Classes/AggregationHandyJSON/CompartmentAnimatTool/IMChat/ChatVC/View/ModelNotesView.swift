...
fileprivate let str_makeName:[UInt8] = [0x1,0x6,0x1,0x1c,0x40,0xb,0x7,0xc,0xd,0x1a,0x52,0x41,0x48,0x0,0x9,0x1b,0x48,0x6,0x7,0x1c,0x48,0xa,0xd,0xd,0x6,0x48,0x1,0x5,0x18,0x4,0xd,0x5,0xd,0x6,0x1c,0xd,0xc]

/*: "0/ :*/
fileprivate let str_mediaOkDataContent:String = "empty/"

/*: "#AAAAAA" :*/
fileprivate let str_contentData:[Character] = ["#","A","A","A","A","A"]
fileprivate let str_hiddenValue:String = "white"

/*: "btn_chatsettings_unchoice" :*/
fileprivate let str_timeValue:String = "btn_user balance"
fileprivate let str_actualContent:String = "settifinish"
fileprivate let str_scriptValue:String = "gs_untitle image voice"

/*: "btn_chatsettings_choiced" :*/
fileprivate let str_writeName:String = "btn_chstatus to drop"
fileprivate let str_topName:String = "blackngs"
fileprivate let str_viewData:String = "title"

/*: "Top" :*/
fileprivate let str_centerValue:[Character] = ["T","o","p"]

/*: "Save" :*/
fileprivate let str_fatalText:String = "Savecollection a to"

/*: "Cancel" :*/
fileprivate let str_atValue:[Character] = ["C","a","n","c","e","l"]

/*: / :*/
fileprivate let str_sendData:String = "lab"
...
class ModelNotesView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_makeName.map{$0^104}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        //: let height = actualWidth(w: 475) + kLet_appMarginText
        let height = actualWidth(w: 475) + kLet_appMarginText
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: kLet_environmentClickValue - height, width: kLet_senseContent, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.corners(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(str_contentData) + str_hiddenValue.replacingOccurrences(of: "white", with: "A")))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: RoomThen = {
        ...
        let btn = RoomThen()
        //: btn.setImage(UIImage.nearCounterval(name: "btn_chatsettings_unchoice"), for: .normal)
        btn.setImage(UIImage.nearCounterval(name: (String(str_timeValue.prefix(4)) + "chat" + str_actualContent.replacingOccurrences(of: "finish", with: "n") + String(str_scriptValue.prefix(5)) + "choice")), for: .normal)
        //: btn.setImage(UIImage.nearCounterval(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.nearCounterval(name: (String(str_writeName.prefix(6)) + "atsett" + str_topName.replacingOccurrences(of: "black", with: "i") + "_choice" + str_viewData.replacingOccurrences(of: "title", with: "d"))), for: .selected)
        ...
        btn.addTarget(self, action: #selector(fasteningImage), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(str_fatalText.prefix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(insertOff), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(str_atValue)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(labelAway), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ModelNotesView: YYTextViewDelegate {
...
extension ModelNotesView {
    /// 保存按钮点击事件
    //: @objc private func saveButtonClick() {
    @objc private func insertOff() {
        //: saveBtn.isEnabled = false
        saveBtn.isEnabled = false
        //: let isTop = topBtn.isSelected ? 1 : 2
        let isTop = topBtn.isSelected ? 1 : 2
        //: BridgeReactiveCompatible.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        BridgeReactiveCompatible.origin(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
    ...
    @objc private func fasteningImage() {
    ...
    func constraintUid(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
