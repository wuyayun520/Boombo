...
fileprivate let str_libraryValue:String = "live managerDocu"
fileprivate let str_showData:String = "User/price raw report"

/*: "Documents/User/record/" :*/
fileprivate let str_touchData:[Character] = ["D","o","c","u","m","e"]
fileprivate let str_countData:String = "nts/error user"
fileprivate let str_stopValue:String = "bag class tableecord/"

/*: "cancelled" :*/
fileprivate let str_colorTitle:[Character] = ["c","a","n","c","e","l","l","e","d"]

/*: .wav" :*/
fileprivate let str_loopTitle:String = "pop self model button.wav"
...
typealias OneKeyGreetDownloadBlock = (_ model: ManagerThen) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol TitleObjectProtocol: NSObjectProtocol {
    /// 下载完成
    ...
    func sprechgesang(model: ManagerThen)
    /// 过期语音消息，服务器缓存时间为7天
...
extension HandleSessionDelegate {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func headAdd(_ tasksArray: [ManagerThen]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.getPath()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(str_libraryValue.suffix(4)) + "ments/" + String(str_showData.prefix(5)) + "voice/")) {
                //: model.zipFilePath = "\(ToDoingReactiveCompatible.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(ToDoingReactiveCompatible.courseOfActionValue())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(str_touchData) + String(str_countData.prefix(4)) + "User/r" + String(str_stopValue.suffix(6)))) {
                //: model.zipFilePath = "\(ToDoingReactiveCompatible.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(ToDoingReactiveCompatible.allPath())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            model.taskStatus = .prepare
            //: MarkThen.db_updateVoiceMsg(msgId: model.taskId, status: "\(DownloadTaskStatus.prepare.rawValue)", filePath: model.zipFilePath)
            MarkThen.refreshOfPath(msgId: model.taskId, status: "\(PopCustomReflectable.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func locationCell(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = ManagerThen.doingeBind(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? ManagerThen()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(str_libraryValue.suffix(4)) + "ments/" + String(str_showData.prefix(5)) + "voice/")) {
            //: taskModel.zipFilePath = "\(ToDoingReactiveCompatible.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(ToDoingReactiveCompatible.courseOfActionValue())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(str_touchData) + String(str_countData.prefix(4)) + "User/r" + String(str_stopValue.suffix(6)))) {
            //: taskModel.zipFilePath = "\(ToDoingReactiveCompatible.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(ToDoingReactiveCompatible.allPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension HandleSessionDelegate: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = ManagerThen.popFrom(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.sprechgesang(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (String(str_colorTitle)) {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    addSuspend(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.direction(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                addSuspend(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.duringSub(url: task.taskDescription ?? "")
        }
...
extension HandleSessionDelegate {
    ...
    func addSuspend(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(PopCustomReflectable.pause.rawValue)" : "\(PopCustomReflectable.error.rawValue)"
            //: MarkThen.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            MarkThen.refreshOfPath(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.inscription(model: taskModel)
            }
    ...
    func direction(url: String) {
        ...
        //: case .chatRecord:
        case .chatRecord:
            //: MarkThen.db_deleteVoiceMsg(msgId: taskModel.taskId)
            MarkThen.resource(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.download(model: taskModel)
    ...
    func duringSub(url: String) {
        ...
        let size = ManagerThen.popFrom(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            //: if taskModel.taskTypeId == .chatRecord {
            if taskModel.taskTypeId == .chatRecord {
                //: MarkThen.db_updateVoiceMsg(msgId: taskModel.taskId, status: "\(DownloadTaskStatus.finish.rawValue)")
                MarkThen.refreshOfPath(msgId: taskModel.taskId, status: "\(PopCustomReflectable.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(str_loopTitle.suffix(4)))
            ...
            self.deleteOf(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.trademarkRoot(model: taskModel)
            ...
            //: case .chatRecord:
            case .chatRecord:
                //: MarkThen.db_updateVoiceMsg(msgId: taskModel.taskId, status: "\(DownloadTaskStatus.expired.rawValue)")
                MarkThen.refreshOfPath(msgId: taskModel.taskId, status: "\(PopCustomReflectable.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.number(model: taskModel)
    ...
    func deleteOf(at path: String, withKeyId keyId: String, withTypeId typeId: StackEffectExpressionConvertible, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(str_loopTitle.suffix(4)))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    if typeId == .chatRecord {
                        //: MarkThen.db_updateVoiceMsg(msgId: keyId, filePath: newPath)
                        MarkThen.refreshOfPath(msgId: keyId, filePath: newPath)
                    }
