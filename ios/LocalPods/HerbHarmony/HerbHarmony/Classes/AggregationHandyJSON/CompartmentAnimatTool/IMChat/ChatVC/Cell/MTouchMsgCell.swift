...
/*: "icon_lounge" :*/
fileprivate let str_actionName:[Character] = ["i","c","o","n","_","l"]
fileprivate let str_viewValue:String = "OUNGE"

/*: "icon_talk_points" :*/
fileprivate let str_retainTitle:String = "backgroundc"
fileprivate let str_modelAddValue:String = "return nameon_tal"

/*: "C6BDFF" :*/
fileprivate let str_inputValue:[Character] = ["C","6","B","D","F","F"]

/*: "#BBBBBB" :*/
fileprivate let str_backgroundName:String = "#BBBBBBif list send"

/*: "Reply to get points~" :*/
fileprivate let str_timeName:[Character] = ["R","e","p","l","y"," ","t","o"," ","g","e","t"," ","p","o"]
fileprivate let str_numberName:String = "return data handle color closeints~"

/*: "bth_unread_nor" :*/
fileprivate let str_textValue:String = "message make voice nearbth_"
fileprivate let str_clickData:String = "oarray"

/*: "icon_male_default" :*/
fileprivate let str_showContent:[UInt8] = [0xe1,0xeb,0xe7,0xe6,0xd7,0xe5,0xe9,0xe4,0xed,0xd7,0xec,0xed,0xee,0xe9,0xfd,0xe4,0xfc]

private func viewShare(text num: UInt8) -> UInt8 {
    return num ^ 136
}

/*: "icon_female_default" :*/
fileprivate let str_languageName:String = "color any else match touchicon_f"
fileprivate let str_toTitle:String = "_defauplayer view"
fileprivate let str_videoData:[Character] = ["l","t"]

/*: "   " :*/
fileprivate let str_totalValue:String = "   "

/*: "+%@ points" :*/
fileprivate let str_countValue:String = "+%@ polet return if"

/*: "bth_read_pre" :*/
fileprivate let str_pointValue:String = "let white showbth_r"
fileprivate let str_tabValue:[Character] = ["e","a","d","_","p","r","e"]
...
class MTouchMsgCell: TUIMessageCell {
    //: var cellData: TalkingChatMsgBaseCellData?
    var cellData: CollectionReactiveCompatible?
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.nearCounterval(name: (String(str_actionName) + str_viewValue.lowercased()))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var ageLab: UILabel = {
        ...
        label.textColor = .white
        //: label.font = UIFont.behindUpRoom(fontSize: 10)
        label.font = UIFont.behindUpRoom(fontSize: 10)
        //: return label
        return label
        //: }()
    ...
    public lazy var coinIconImg: UIImageView = {
        ...
        //: imgV.isHidden = true
        imgV.isHidden = true
        //: imgV.image = UIImage.nearCounterval(name: "icon_talk_points")
        imgV.image = UIImage.nearCounterval(name: (str_retainTitle.replacingOccurrences(of: "background", with: "i") + String(str_modelAddValue.suffix(6)) + "k_points"))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    public lazy var coinLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(str_inputValue)))
        //: label.isHidden = true
        label.isHidden = true
    ...
    public lazy var replyTipLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(str_backgroundName.prefix(7))))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.unprecedented(fontSize: 14)
        label.font = UIFont.unprecedented(fontSize: 14)
        //: label.text = "Reply to get points~".localized
        label.text = (String(str_timeName) + String(str_numberName.suffix(5))).localized
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var isReadImg: UIImageView = {
        ...
        //: imgV.isHidden = true
        imgV.isHidden = true
        //: imgV.image = UIImage.nearCounterval(name: "bth_unread_nor")
        imgV.image = UIImage.nearCounterval(name: (String(str_textValue.suffix(4)) + "unread_n" + str_clickData.replacingOccurrences(of: "array", with: "r")))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension MTouchMsgCell {
    ...
    override func fill(with data: TCommonCellData) {
        ...
        self.cellData = data as? CollectionReactiveCompatible
        ...
        let userModel = newData.msgModel.user
        //: let loungePlus = (WithAdministratorAppManager.share.loginUid == String(userModel.uid)) ? WithAdministratorAppManager.share.loginUserMode.loungePlus : userModel.loungePlus
        let loungePlus = (WithAdministratorAppManager.share.loginUid == String(userModel.uid)) ? WithAdministratorAppManager.share.loginUserMode.loungePlus : userModel.loungePlus

        //: self.nameLabel.text = userModel.nickname
        self.nameLabel.text = userModel.nickname
        //: self.nameLabel.textColor = loungePlus ? .userVipColor() : .appValueColor()
        self.nameLabel.textColor = loungePlus ? .insert() : .colorPush()
        //: self.nameLabel.font = UIFont.behindUpRoom(fontSize: 13)
        self.nameLabel.font = UIFont.behindUpRoom(fontSize: 13)
        //: let imgStr = userModel.sex == Int(Gender.male.rawValue) ? "icon_male_default" : "icon_female_default"
        let imgStr = userModel.sex == Int(MiniSubqueryConvertible.male.rawValue) ? String(bytes: str_showContent.map{viewShare(text: $0)}, encoding: .utf8)! : (String(str_languageName.suffix(6)) + "emale" + String(str_toTitle.prefix(6)) + String(str_videoData))
        //: sexIconBtn.setBackgroundImage(UIImage.nearCounterval(name: imgStr), for: .normal)
        sexIconBtn.setBackgroundImage(UIImage.nearCounterval(name: imgStr), for: .normal)
        ...
        self.avatarView.nearSquare(urlStr: userModel.headPic)
        ...
        self.iconBorder.torso(urlStr: userModel.headPicFrame)

        //: let isShowCoin = newData.msgIncome > 0
        let isShowCoin = newData.msgIncome > 0
        //: self.coinIconImg.isHidden = !(isShowCoin && WithAdministratorAppManager.share.appStatus == AppSkinStatus.normal.rawValue)
        self.coinIconImg.isHidden = !(isShowCoin && WithAdministratorAppManager.share.appStatus == SectionFreckleTextConvertible.normal.rawValue)
        ...
        self.coinLabel.text = (String(str_countValue.prefix(6)) + "ints").localizedArguments(NSNumber(value: Float(newData.msgIncome)))
        // 女性逻辑, 未回复付费消息提示
        //: if TalkingPrivateChatManager.needShowReplyTipsMessage(newData.direction,
        if HandleThen.screen(newData.direction,
        ...
        if HandleThen.photoData(cellData: newData) {
            //: self.isReadImg.isHidden = false
            self.isReadImg.isHidden = false
            //: var readImgStr = "bth_unread_nor"
            var readImgStr = (String(str_textValue.suffix(4)) + "unread_n" + str_clickData.replacingOccurrences(of: "array", with: "r"))
            // 消息对端是否已读
            //: if TalkingPrivateChatManager.msgIsRead(cellData: newData) {
            if HandleThen.beyondCollection(cellData: newData) {
                //: readImgStr = "bth_read_pre"
                readImgStr = (String(str_pointValue.suffix(5)) + String(str_tabValue))
            }
            //: self.isReadImg.image = UIImage.nearCounterval(name: readImgStr)
            self.isReadImg.image = UIImage.nearCounterval(name: readImgStr)
    ...
    override public func updateConstraints() {
        ...
        if let temCellData = self.cellData {
            //: loungePlus = (WithAdministratorAppManager.share.loginUid == String(temCellData.msgModel.user.uid)) ? WithAdministratorAppManager.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
            loungePlus = (WithAdministratorAppManager.share.loginUid == String(temCellData.msgModel.user.uid)) ? WithAdministratorAppManager.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
        }
