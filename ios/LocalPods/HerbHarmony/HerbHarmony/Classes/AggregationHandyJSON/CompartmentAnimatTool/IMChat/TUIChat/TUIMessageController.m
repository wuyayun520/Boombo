...
- (void)setupViews {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(menuWillHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onNewMessage:) name:[TouchMiniThen targetOf] object:nil];
...
- (void)loadMessage {
    ...
    if (self.currentChatType == TalkingIMChatType_group ||
        ...
        [[V2TIMManager sharedInstance] getGroupHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([EdificeV2Listener shared].isConnection) { //推送点进来会报错
                NSString *str = [@"Net Error, Try again later" localized];
                [self atShowWithShowMsg:[str localized]];
            }
        ...
        [[V2TIMManager sharedInstance] getC2CHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([EdificeV2Listener shared].isConnection) {                
                if (WithAdministratorAppManager.share.appStatus == 0) {
                    NSString *str = [@"Net Error, Try again later" localized];
                    [self atShowWithShowMsg:[str localized]];
                }
            }
...
- (void)getMessages:(NSArray *)msgs msgCount:(int)msgCount {
    ...
    for (NSInteger k = array.count - 1; k >= 0; --k) {
        ...
        if ([userId isEqualToString:WithAdministratorAppManager.share.loginUserMode.userID]) {
...
- (void)onNewMessage:(NSNotification *)notification
{
    ...
    if ([msg.sender isEqualToString:WithAdministratorAppManager.share.loginUserMode.userID]) {
        //过滤本人发送的消息
        return;
    }
    ...
    if (uiMsgs.count > 0) {
        ...
        if ((self.tableView.contentSize.height - self.tableView.contentOffset.y) < [IconThen deleteGift] * 1.5) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    缓存存在问题待处理
    //    if (height > 0 && ![data isKindOfClass:[AbClickThen class]] && ![data isKindOfClass:[ShowCellData class]]) {
    //        return height;
    //    }
...
- (void)sendCustomMessage:(TUIMessageCellData *)msg {
    ...
    NSInteger sender = WithAdministratorAppManager.share.loginUserMode.userID.integerValue;
    NSString *msgID;
    if (isGroup) {
...
- (void)modifyCustomMessageDataWithExtral:(NSDictionary *)extral cellData:(TUIMessageCellData *)msg {
    ...
    // 更新内存模型
    msg.msgModel = [DataNameModel modelWithDic:newExtraDict];
}
...
///   - msg: 图片消息
///   - model: 私密照片模型
- (void)changeIntimatePhotoMsg:(TUIMessageCellData *)msg photoModel:(HypothesisModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [HypothesisModel stain:model giftId:msg.msgID];
...
///   - msg: 视频消息
///   - model: 私密视频模型
- (void)changeIntimateVideoMsg:(TUIMessageCellData *)msg videoModel:(ValueJsonModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [ValueJsonModel neighbour:model data:msg.msgID];
...
- (void)changeIntimateGiftMsg:(TUIMessageCellData *)msg {
    dispatch_async(dispatch_get_main_queue(), ^{
        [TelecastingModel priceId:msg.msgID];
...
- (TalkingSystemMsgCellData *)transSystemMsgFromDate:(NSDate *)date
{
    if(_msgForDate == nil || fabs([date timeIntervalSinceDate:_msgForDate.timestamp]) > MAX_MESSAGE_SEP_DLAY) {
        ...
        system.contentStr = [NSDate changeCharacterWithShowDetail:YES date:date];
...
- (void)onLongPressMessageWithCell:(TUIMessageCell *)cell{
    ...
    if ([data isKindOfClass:[CollectionThen class]]) {
        CollectionThen *textCellData = (CollectionThen *)data;
    ...
    // 只有文字才有复制功能
    if (([data isKindOfClass:[DirectorThen class]] ||
         [data isKindOfClass:[CollectionThen class]]) && hasJumps == NO) {
        // 过滤掉音视频通话文本消息
...
- (NSArray *)requiredResponseWithLongGestureRecognizer {
    return @[[DirectorThen class],
             [CollectionThen class],
             [VoxCellData class],
             [IndexClearThen class],
             [DemonstrateCellData class],
             [ButtonCellData class],
             [QuantityCellData class],
             [UserCellData class]];
}
...
- (void)onCopyMsg:(id)sender {
    ...
    if ([_menuUIMsg isKindOfClass:[DirectorThen class]] ||
        [_menuUIMsg isKindOfClass:[CollectionThen class]]) {
        CollectionThen *txtMsg = (CollectionThen *)_menuUIMsg;
...
- (BOOL)canReplyResponseWithLongGestureRecognizer:(TUIMessageCellData *)cellData {
    ...
    if ([cellData isKindOfClass:[ButtonCellData class]] || [cellData isKindOfClass:[VoxCellData class]] || [cellData isKindOfClass:[UserCellData class]]) {
        return YES;
    }
...
- (void)translateMsgWithCellData:(TUIMessageCellData *)cellData {
    ...
    if (WithAdministratorAppManager.share.appUserConfigMode.enableTranslate == NO) { // 非会员，无法使用翻译
    ...
    } else if ([cellData isKindOfClass:[DirectorThen class]]) {
        DirectorThen *txtMsg = (DirectorThen *)cellData;
...
- (void)translateMessageToReq:(TUIMessageCellData *)txtMsg andType:(int)type {
    ...
    // 调用翻译接口
    [BridgeReactiveCompatible toSectionWithMsgId:txtMsg.msgModel.msgInfo.msgId type:type completion:^(BOOL succeed, NSDictionary *dict, NSInteger errorCode) {
        if (!succeed) {
            if (errorCode == 403) { // 非会员，无法使用翻译
                WithAdministratorAppManager.share.appUserConfigMode.enableTranslate = NO;
                [[LeadingEqualReactiveCompatible share] numberimateVc];
            }
