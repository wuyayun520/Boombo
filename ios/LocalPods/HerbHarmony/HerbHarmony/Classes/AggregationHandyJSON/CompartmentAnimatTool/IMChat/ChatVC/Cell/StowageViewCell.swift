...
fileprivate let str_leadingData:[UInt8] = [0x7f,0x78,0x7f,0x62,0x3e,0x75,0x79,0x72,0x73,0x64,0x2c,0x3f,0x36,0x7e,0x77,0x65,0x36,0x78,0x79,0x62,0x36,0x74,0x73,0x73,0x78,0x36,0x7f,0x7b,0x66,0x7a,0x73,0x7b,0x73,0x78,0x62,0x73,0x72]

private func indexGift(view num: UInt8) -> UInt8 {
    return num ^ 22
}

/*: "all" :*/
fileprivate let str_presentData:[Character] = ["a","l","l"]
...
class StowageViewCell: MTouchMsgCell {
    //: var giftData: LanthanideCellData?
    var giftData: LanthanideCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_leadingData.map{indexGift(view: $0)}, encoding: .utf8)!)
    }
...
extension StowageViewCell {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)
        //: self.giftData = data as? LanthanideCellData
        self.giftData = data as? LanthanideCellData
        ...
        self.giftImgView.nearSquare(urlStr: imgPreview)
        //: self.giftContentLab.attributedText = giftData.attributedString
        self.giftContentLab.attributedText = giftData.attributedString
        //: if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
        if giftData.msgModel.toUser.uid == (String(str_presentData)) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.nearCounterval(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.nearCounterval(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.nearSquare(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
