...
fileprivate let str_blockPageValue:[UInt8] = [0xc1,0xc6,0xc1,0xcc,0x80,0xbb,0xc7,0xbc,0xbd,0xca,0x92,0x81,0x78,0xc0,0xb9,0xcb,0x78,0xc6,0xc7,0xcc,0x78,0xba,0xbd,0xbd,0xc6,0x78,0xc1,0xc5,0xc8,0xc4,0xbd,0xc5,0xbd,0xc6,0xcc,0xbd,0xbc]

fileprivate func travelRapidlyRoot(number num: UInt8) -> UInt8 {
...
/*: "icon_chat_lock" :*/
fileprivate let str_toText:String = "icon_chsearch interval title var label"
fileprivate let str_indexValue:String = "partyt"
fileprivate let str_managerShowContent:String = "flexible self view_lock"

/*: "icon_chat_burned" :*/
fileprivate let str_colorValue:String = "unknown type sizeicon_c"
fileprivate let str_viewHiddenData:String = "urtableed"

/*: "icon_chat_burned_shan" :*/
fileprivate let str_timeData:String = "icon_chatrow table return near"
fileprivate let str_locationTitle:String = "make false var_burned"
fileprivate let str_viewActualData:String = "_shanview add add to"

/*: "Already burned" :*/
fileprivate let str_loveText:String = "Alreadyimage name"
fileprivate let str_toTitle:String = "true after range burned"
...
class ExtraMsgCell: MTouchMsgCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_blockPageValue.map{travelRapidlyRoot(number: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        //: imag.alpha = 0.9
        imag.alpha = 0.9
        //: imag.image = UIImage.nearCounterval(name: "icon_chat_lock")
        imag.image = UIImage.nearCounterval(name: (String(str_toText.prefix(7)) + str_indexValue.replacingOccurrences(of: "party", with: "a") + String(str_managerShowContent.suffix(5))))
    ...
    lazy var burnedImagV: RoomThen = {
        ...
        imgV.setBackgroundImage(UIImage.nearCounterval(name: (String(str_colorValue.suffix(6)) + "hat_b" + str_viewHiddenData.replacingOccurrences(of: "table", with: "n"))), for: .normal)
        //: imgV.setImage(UIImage.nearCounterval(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.nearCounterval(name: (String(str_timeData.prefix(9)) + String(str_locationTitle.suffix(7)) + String(str_viewActualData.prefix(5)))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.spaceBetweenTitleAndImage = 20
        imgV.spaceBetweenTitleAndImage = 20
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(str_loveText.prefix(7)) + String(str_toTitle.suffix(7))).localized, for: .normal)
...
extension ExtraMsgCell {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)
        //: guard let imageCellData = data as? IndexClearThen else { return }
        guard let imageCellData = data as? IndexClearThen else { return }

        //: self.photoView.backgroundColor = .white
        self.photoView.backgroundColor = .white
        //: let intimatePhoto: HypothesisModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        let intimatePhoto: HypothesisModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            statuteText(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            statuteText(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: IndexClearThen, urlStr: String?, isMosaic: Bool) {
    func statuteText(data: IndexClearThen, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.error(image, size: 90)
                        self.photoView.image = UIImage.error(image, size: 90)
                        //: } else {
                    } else {
            ...
            pictureGallery(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.nearSquare(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func pictureGallery(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.oreoCookieWhiteCompletion(urlStr: urlStr) { [weak self] image in
