...
/*: "call_bgm" :*/
fileprivate let str_playerPriceVarName:[Character] = ["c","a","l","l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let str_actualText:[Character] = ["c","a","l","l","_","r","e","s","p","o"]
fileprivate let str_valueData:String = "true self text computer rownse_bgm"
...
class InfoEqualCallPlayer: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = VoiceFromInsetTarget.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func baseballPlayer(type: VoiceFromInsetTarget) -> InfoEqualCallPlayer {
        //: let player = TalkingCallBGMPlayer()
        let player = InfoEqualCallPlayer()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.equalPath()
...
extension InfoEqualCallPlayer {
    ...
    private func sendNotify(notify: Notification) {
    ...
    private func accumulationFractal(notify: Notification) {
    ...
    private func equalPath() {
        ...
        var fileName = (String(str_playerPriceVarName))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(str_actualText) + String(str_valueData.suffix(7)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = DigitiserReactiveCompatible.default.postType(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension InfoEqualCallPlayer: TXVodPlayListener {
