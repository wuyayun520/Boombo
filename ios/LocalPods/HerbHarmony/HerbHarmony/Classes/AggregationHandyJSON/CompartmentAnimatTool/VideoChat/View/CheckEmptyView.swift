...
fileprivate let str_picData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#FFD13A" :*/
fileprivate let str_numberText:String = "true var#FFD13A"

/*: "gift_gb_umnad_1" :*/
fileprivate let str_rowAdValue:[Character] = ["g","i","f","t","_"]
fileprivate let str_titleData:String = "image array return letgb_umn"

/*: "icon_circle_pic_default" :*/
fileprivate let str_liveValue:String = "icon_cirshow label any view"
fileprivate let str_genderColorMakeData:String = "modelle"
fileprivate let str_gestureTitle:[Character] = ["_","p","i","c","_","d","e","f","a","u","l","t"]

/*: "Open lucky blind-box" :*/
fileprivate let str_topValue:String = "Open user data view text open"
fileprivate let str_partyNameReplacementData:String = "text let to half blind-"
fileprivate let str_buttonName:String = "infox"

/*: "Sent " :*/
fileprivate let str_fillValue:String = "Sent if to"

/*: "PingFangSC-Semibold" :*/
fileprivate let str_appContent:String = "let to self indexPingF"
fileprivate let str_columnValue:[Character] = ["e"]
fileprivate let str_photoValue:[Character] = ["m","i","b","o","l","d"]

/*: "%d" :*/
fileprivate let str_streamData:String = "%dwrap self equal if"

/*: "Bombard" :*/
fileprivate let str_keyData:String = "Bombarwrap self to"
fileprivate let str_sendUserData:[Character] = ["d"]
...
class CheckEmptyView: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = AggregationHandyJSON()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [ButtonModelType()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_picData.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var nickLb: UILabel = {
        ...
        //: lb.numberOfLines = 1
        lb.numberOfLines = 1
        //: lb.font = .unprecedented(fontSize: 14)
        lb.font = .unprecedented(fontSize: 14)
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(str_numberText.suffix(7))))
    ...
    private lazy var pluralTip: BackGiftItemLabel = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = BackGiftItemLabel()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: BackGiftItemLabel = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = BackGiftItemLabel()
...
extension CheckEmptyView {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func itemEqual(model: AggregationHandyJSON) {
        //: self.giftMsgModel = model
        self.giftMsgModel = model
        //: let fromNickname = giftMsgModel.fromNickname
        let fromNickname = giftMsgModel.fromNickname
        //: self.bgImage.image = UIImage.nearCounterval(name: "gift_gb_umnad_1")
        self.bgImage.image = UIImage.nearCounterval(name: (String(str_rowAdValue) + String(str_titleData.suffix(6)) + "ad_1"))

        //: let placeholder = UIImage.nearCounterval(name: "icon_circle_pic_default")
        let placeholder = UIImage.nearCounterval(name: (String(str_liveValue.prefix(8)) + str_genderColorMakeData.replacingOccurrences(of: "model", with: "c") + String(str_gestureTitle)))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == ExamineVarArg.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(str_topValue.prefix(5)) + "lucky" + String(str_partyNameReplacementData.suffix(7)) + str_buttonName.replacingOccurrences(of: "info", with: "bo")).localized
            //: }else {
        } else {
        ...
        self.avatarImage.commentGesture(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.commentGesture(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(str_fillValue.prefix(5))).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(str_appContent.suffix(5)) + "angSC-S" + String(str_columnValue) + String(str_photoValue)), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = statusName(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(str_appContent.suffix(5)) + "angSC-S" + String(str_columnValue) + String(str_photoValue)), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = statusName(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.listMenu(texts: strs, colors: colors, fonts: fonts)
        ...
        makeSearched(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func makeSearched(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(str_appContent.suffix(5)) + "angSC-S" + String(str_columnValue) + String(str_photoValue)), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = statusName(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(str_appContent.suffix(5)) + "angSC-S" + String(str_columnValue) + String(str_photoValue)), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = statusName(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.equalRgba(150, 77, 20, 1), UIColor.white, UIColor.equalRgba(150, 77, 20, 1)]
        ...
        attText = attText.listMenu(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.quoteWithSearched(attributedText: attText)
        ...
        countLb.prepose(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func addOff(giftMsgModel: AggregationHandyJSON) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(wearer), object: nil)
        ...
        self.perform(#selector(wearer), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func exhibitAnimate(model _: AggregationHandyJSON, animate _: Bool) {
    ...
    @objc func wearer() {
        ...
        giftMsgModel = AggregationHandyJSON()
...
extension CheckEmptyView {
    ...
    func statusName(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension CheckEmptyView {
    ...
    private func loadSubviews() {
        ...
        font = statusName(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.listMenu(texts: [(String(str_keyData.prefix(6)) + String(str_sendUserData)).localized], colors: [UIColor.equalRgba(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.prepose(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.quoteWithSearched(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.prepose(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
