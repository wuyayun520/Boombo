...
fileprivate let str_modeCellTitle:[UInt8] = [0xe1,0xe6,0xe1,0xec,0xa0,0xdb,0xe7,0xdc,0xdd,0xea,0xb2,0xa1,0x98,0xe0,0xd9,0xeb,0x98,0xe6,0xe7,0xec,0x98,0xda,0xdd,0xdd,0xe6,0x98,0xe1,0xe5,0xe8,0xe4,0xdd,0xe5,0xdd,0xe6,0xec,0xdd,0xdc]

fileprivate func textModel(fatal num: UInt8) -> UInt8 {
...
class ShowView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_modeCellTitle.map{textModel(fatal: $0)}, encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        DataSocketDelegate.shared.containerView?.isHidden = false
    }
    ...
    func network() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= kLet_senseContent * 0.5) ? -self.width : kLet_senseContent + self.width
            ...
            DataSocketDelegate.shared.containerView?.isHidden = true
        }
    }
}
...
extension ShowView {
    ...
    func tapGesture() -> UITapGestureRecognizer {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.counteractionUpSender(sender:)))
    ...
    func gestures() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.courseOfAction(sender:)))
    ...
    @objc func courseOfAction(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(kLet_senseContent - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(kLet_environmentClickValue - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = kLet_senseContent - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = kLet_senseContent
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= kLet_senseContent * 0.5 {
            ...
            DataSocketDelegate.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func counteractionUpSender(sender _: UIPanGestureRecognizer) {
