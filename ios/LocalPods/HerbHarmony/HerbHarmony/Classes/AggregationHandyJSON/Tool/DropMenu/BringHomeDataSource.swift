...
fileprivate let str_titleData:[UInt8] = [0xe4,0xe3,0xe4,0xf9,0xa5,0xee,0xe2,0xe9,0xe8,0xff,0xb7,0xa4,0xad,0xe5,0xec,0xfe,0xad,0xe3,0xe2,0xf9,0xad,0xef,0xe8,0xe8,0xe3,0xad,0xe4,0xe0,0xfd,0xe1,0xe8,0xe0,0xe8,0xe3,0xf9,0xe8,0xe9]

private func knockOffEffect(game num: UInt8) -> UInt8 {
    return num ^ 141
}

/*: "SHBDropMenuCell" :*/
fileprivate let str_modeValue:String = "room return view max nearSHBDr"
fileprivate let str_makeName:String = "content type model table blocknuCell"

/*: "title" :*/
fileprivate let str_toTitle:[Character] = ["t","i","t","l","e"]
...
public class BringHomeDataSource: UIView {
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.SinkTargetLeading.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.ConfigManager.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.ConfigManager.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.ConfigManager.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? ModeMMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
    ...
    public class func stopAdd(anchorView: UIView, titleArray: [String], imageArray: [UIImage]) -> BringHomeDataSource {
        ...
        let menuView = tableArray(anchorView: anchorView, menuArray: menuArray)
        //: return menuView
        return menuView
    }

    //: public class func pullDropDrownMenu(anchorView: UIView, menuArray: [DropMenuModel]) -> DropDownMenuView {
    public class func tableArray(anchorView: UIView, menuArray: [ToCountimateMenuModel]) -> BringHomeDataSource {
        //: let window = IconThen.getWindow()
        let window = IconThen.getBind()
        //: let menuView = DropDownMenuView()
        let menuView = BringHomeDataSource()
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_titleData.map{knockOffEffect(game: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func academiaFuturismBar() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func viewAcross() -> UITableView {
    ...
    private func talkPlayer() {
        //: addSubview(contentView)
        addSubview(contentView)
        //: mTable.register(DropMenuCell.self, forCellReuseIdentifier: "SHBDropMenuCell")
        mTable.register(TableDHoteViewCell.self, forCellReuseIdentifier: (String(str_modeValue.suffix(5)) + "opMe" + String(str_makeName.suffix(6))))
        //: contentView.addSubview(mTable)
        contentView.addSubview(mTable)
    }

    //: private func refreshUI() {
    private func postGreet() {
        //: drawmTableFrame()
        cancelData()
        //: mTable.reloadData()
        mTable.reloadData()
...
extension BringHomeDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(str_modeValue.suffix(5)) + "opMe" + String(str_makeName.suffix(6))), for: indexPath) as! TableDHoteViewCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (String(str_toTitle)))
        //: animateRemoveView()
        nameStart()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.postPush(index: indexPath.row, title: cellModel.title ?? "")
    }
}
