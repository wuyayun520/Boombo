...
/*: "gifts" :*/
fileprivate let str_intimateContent:String = "videofts"

/*: "vipGifts" :*/
fileprivate let str_conversationName:String = "vipGiftsvalue string"

/*: "packGifts" :*/
fileprivate let str_valueTitle:String = "let text forpackGi"
fileprivate let str_fatalTitle:String = "FTS"

/*: "giftSendItem" :*/
fileprivate let str_giftValue:[Character] = ["g","i","f","t","S","e","n","d","I","t","e","m"]
...
class SumReactiveCompatible: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [CustomInsetTarget: [Any]]]()
    // 数据锁
    ...
    private var giftType = ScaleLargessComparable.normal

    //: static let share = TalkingChatGiftManager()
    static let share = SumReactiveCompatible()
...
extension SumReactiveCompatible {
    /// 发送礼物面板数据
    /// - Parameters:
    ...
    func dialog(type: ScaleLargessComparable = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        SumReactiveCompatible.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[CustomInsetTarget.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            SumReactiveCompatible.share.backLike(completion: { isSucceed in
            ...
            SumReactiveCompatible.share.tableReply(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            SumReactiveCompatible.share.get(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            SumReactiveCompatible.share.grapple(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func grapple(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = SumReactiveCompatible.share.giftType
        //: BridgeReactiveCompatible.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        BridgeReactiveCompatible.videoEqual(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [CustomInsetTarget: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(str_intimateContent.replacingOccurrences(of: "video", with: "gi"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<GiftSectionModelType>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[CustomInsetTarget.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(str_conversationName.prefix(8)))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<GiftSectionModelType>.deserialize(from: array as? Array) {
                    ...
                    dataDict[CustomInsetTarget.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(str_valueTitle.suffix(6)) + str_fatalTitle.lowercased())] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<GiftSectionModelType>.deserialize(from: array as? Array) {
                    ...
                    dataDict[CustomInsetTarget.Bags] = bagsArr
                }
                ...
                var firstModel = ButtonModelType()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(str_giftValue))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<ButtonModelType>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func backLike(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = SumReactiveCompatible.share.giftType
        //: BridgeReactiveCompatible.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        BridgeReactiveCompatible.writeCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func completionMemberEnterUponDeviceRoomClickAssemblage(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: BridgeReactiveCompatible.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        BridgeReactiveCompatible.userCompletion(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<PopScaleReactiveCompatible>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func tableReply(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = ScaleLargessComparable.live
        //: BridgeReactiveCompatible.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        BridgeReactiveCompatible.circumferenceMethod(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func get(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = ScaleLargessComparable.party
        //: BridgeReactiveCompatible.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        BridgeReactiveCompatible.observerSend(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func serviceType(perPageCount: Int, dataType: CustomInsetTarget) -> Int {
    ...
    func trailhead(dataType: CustomInsetTarget) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func slight(dataType: CustomInsetTarget, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension SumReactiveCompatible {
    ...
    @objc private func thrusting() {
        //: gift_clearMemoryCache(showRed: true)
        isTotal(showRed: true)
    }
}
...
extension SumReactiveCompatible {
    ...
    func isTotal(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        SumReactiveCompatible.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
