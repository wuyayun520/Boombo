...
fileprivate let str_selectValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Reply" :*/
fileprivate let str_succeedData:String = "float var pop regularReply"

/*: "#F95151" :*/
fileprivate let str_estimatedValue:[Character] = ["#","F"]
fileprivate let str_roomFrameValue:[Character] = ["9","5","1","5","1"]
...
let kLet_liveValue = (kLet_messageContent + 80.0)
...
class QuantityView: UIView {
    ...
    var startDismissBlock: ((_ model: MediaConversationModel) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: QuantityView) -> Void)? // 完成移除视图Block
    //: private var currModel = TalkingConversationModel()
    private var currModel = MediaConversationModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_selectValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        let label = UILabel()
        //: label.font = .liveAttention(type: .Medium, fontSize: 15)
        label.font = .liveAttention(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.titleLabel?.font = .liveAttention(type: .Medium, fontSize: 15)
        btn.titleLabel?.font = .liveAttention(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(str_succeedData.suffix(5))).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(str_estimatedValue) + String(str_roomFrameValue))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension QuantityView {
    ...
    func afterMake(_ model: MediaConversationModel?) {
    ...
    func appear(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -kLet_liveValue
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func viewEvent() {
        ...
        if currModel.chatType == .service {
            ...
            //: LeadingEqualReactiveCompatible.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            LeadingEqualReactiveCompatible.share.cellRowCompletion(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func largeTotalestine(gesture: UISwipeGestureRecognizer) {
...
extension QuantityView {
    ...
    func nameView() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.commentGesture(urlStr: userInfo.headPic, placeImg: UIImage.footnoteSex(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.torso(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
            ...
            headImgView.image = UIImage.firstMake()
            //: iconBorder.isHidden = true
            iconBorder.isHidden = true
        }

        //: msgShowLabel.attributedText = TalkingChatListTableCell.handleShowC2CDetailTextOfMessage(model: currModel)
        msgShowLabel.attributedText = StandardMainViewCell.pairForData(model: currModel)
    }
    ...
    private func sharedBy() {
        ...
        self.frame = CGRect(x: 0, y: -kLet_liveValue, width: ScreenWidth, height: kLet_liveValue)
    ...
    private func stopImage() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(tapGestureEvent))
        let tap = UITapGestureRecognizer(target: self, action: #selector(viewEvent))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(largeTotalestine))
        //: swipe.direction = .up
        swipe.direction = .up
