...
fileprivate let str_topMText:[UInt8] = [0x54,0x59,0x54,0x5f,0x13,0x4e,0x5a,0x4f,0x50,0x5d,0x25,0x14,0xb,0x53,0x4c,0x5e,0xb,0x59,0x5a,0x5f,0xb,0x4d,0x50,0x50,0x59,0xb,0x54,0x58,0x5b,0x57,0x50,0x58,0x50,0x59,0x5f,0x50,0x4f]

fileprivate func meanSun(of num: UInt8) -> UInt8 {
...
/*: "icon_coinxxx" :*/
fileprivate let str_afterTitle:[Character] = ["i","c","o","n","_","c","o","i","n","x","x","x"]

/*: "icon_gift_num_bg" :*/
fileprivate let str_toName:[Character] = ["i","c","o","n","_"]
fileprivate let str_attentionCellKeyValue:String = "gift_live success white"

/*: "x1" :*/
fileprivate let str_photoValue:String = "color1"

/*: " x :*/
fileprivate let str_onValue:String = " xcountry message"
...
class RoomGiftCell: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = GiftSectionModelType()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_topMText.map{meanSun(of: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        //: let lb = UILabel.init()
        let lb = UILabel()
        //: lb.font = UIFont.liveAttention(type: .Regular, fontSize: 10)
        lb.font = UIFont.liveAttention(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: RoomThen = {
        //: let btn = TalkingButton.init()
        let btn = RoomThen()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.liveAttention(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.liveAttention(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.nearCounterval(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.nearCounterval(name: (String(str_afterTitle))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let img = UIImageView()
        //: let imag = UIImage.nearCounterval(name: "icon_gift_num_bg")
        let imag = UIImage.nearCounterval(name: (String(str_toName) + String(str_attentionCellKeyValue.prefix(5)) + "num_bg"))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        //: lb.text = "x1"
        lb.text = "x1"
        //: lb.font = UIFont.liveAttention(type: .Regular, fontSize: 9)
        lb.font = UIFont.liveAttention(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
...
extension RoomGiftCell {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func observerAccept(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderWidth = 1.0
            //: bgView.layer.borderColor = UIColor.alright().cgColor
            bgView.layer.borderColor = UIColor.alright().cgColor
    ...
    func lab(currenmodel: GiftSectionModelType, giftType: CustomInsetTarget) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.nearSquare(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.nearSquare(urlStr: model.cornerMark, placeImg: nil)
        ...
        paraph.alignment = .center
        //: let attributes = [NSAttributedString.Key.font: UIFont.liveAttention(type: .Regular, fontSize: 11),
        let attributes = [NSAttributedString.Key.font: UIFont.liveAttention(type: .Regular, fontSize: 11),
...
extension RoomGiftCell {
    ...
    func fountainhead() {
        ...
        bgView.backgroundColor = UIColor.spendColor()
