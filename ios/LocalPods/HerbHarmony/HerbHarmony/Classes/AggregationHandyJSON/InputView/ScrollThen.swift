...
fileprivate let str_fieldText:[Character] = ["设","置","普","通","状","态","的","\u{6587}","字","\u{989c}","色","\u{65f6}"," ","请","使","\u{7528}","R"]
fileprivate let str_micData:String = "GB空let let m manager start"
fileprivate let str_observeName:String = "间\u{7684}颜色值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let str_labText:String = "设置\u{9009}\u{4e2d}"
fileprivate let str_backgroundName:String = "颜\u{8272}\u{65f6} "
fileprivate let str_selectedData:String = "GB空间\u{7684}颜色值"

/*: "init(coder:) has not been implemented" :*/
fileprivate let str_addData:[UInt8] = [0x95,0x9a,0x95,0xa0,0x54,0x8f,0x9b,0x90,0x91,0x9e,0x66,0x55,0x4c,0x94,0x8d,0x9f,0x4c,0x9a,0x9b,0xa0,0x4c,0x8e,0x91,0x91,0x9a,0x4c,0x95,0x99,0x9c,0x98,0x91,0x99,0x91,0x9a,0xa0,0x91,0x90]

fileprivate func meMake(pic num: UInt8) -> UInt8 {
...
/*: "设置的下标不合法!!" :*/
fileprivate let str_infoStatusTitle:String = "设置的下\u{6807}"

/*: "Bags" :*/
fileprivate let str_environmentData:String = "true session letBags"
...
open class ScrollThen: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: TargetFrameSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.impressionRgb(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(str_fieldText) + String(str_micData.prefix(3)) + str_observeName))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.impressionRgb(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((str_labText + "状\u{6001}的文字" + str_backgroundName + "请\u{4f7f}用R" + str_selectedData))
        }
    ...
    fileprivate func impressionRgb(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: TargetFrameSegmentStyle, titles: [String]) {
    ...
    @objc func ofFile(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? StreetwiseCustomLabel else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: str_addData.map{meMake(pic: $0)}, encoding: .utf8)!)
    }
}
...
public extension ScrollThen {
    ...
    func outputError(_ titles: [String]) {
        // 移除所有的scrollView子视图
...
//: extension ScrollSegmentView {
extension ScrollThen {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func keyTag() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = StreetwiseCustomLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.ofFile(_:)))
            ...
            if title == (String(str_environmentData.suffix(4))).localized {
    ...
    fileprivate func postAt() {
        ...
        if let firstLabel = labelsArray[0] as? StreetwiseCustomLabel {
    ...
    func channel(_: Bool) {
        ...
        if currentIndex == titles.firstIndex(of: (String(str_environmentData.suffix(4))).localized) {
            //: redBagsLab.isHidden = true
            redBagsLab.isHidden = true
            //: TalkingChatGiftManager.share.showBagsRed = false
            SumReactiveCompatible.share.showBagsRed = false
        }

        //: guard currentIndex != oldIndex else { return }
        guard currentIndex != oldIndex else { return }
        //: let oldLabel = labelsArray[oldIndex] as! CustomLabel
        let oldLabel = labelsArray[oldIndex] as! StreetwiseCustomLabel
        //: let currentLabel = labelsArray[currentIndex] as! CustomLabel
        let currentLabel = labelsArray[currentIndex] as! StreetwiseCustomLabel
        // 让选中标签居中显示
    ...
    public func away(_ currentIndex: Int) {
...
open class StreetwiseCustomLabel: UILabel {
    /// 用来记录当前label的缩放比例
